...
<title><?php echo $info['title'];?>-<?php echo $webname;?></title>
  ...
  <?php echo Common::css("mycss/strategy/TravelNotes.css");?>
  <?php echo Common::js('common/jquery-1.11.3.min.js,common/bootsAitto-min.js,strategy/TravelNotes.js');?>
  <style>
  </style>
...
<section> 
  ...
  <div  class="BreadCrumb">
    <ul >
      ...
      <li><a href="#">游记<i class="icon-gt">&gt;</i></a></li>
      <li class="current"><a href="#">游记正文</a></li>
    </ul>
    <div class="clearfix"></div>
  </div>
  <!--  面包屑 结束 --> 
</section>
<div class="travelFloat-nav">
  <div class="nav-warp">
    ...
    <div class="ui-navArea-btn">
      <ul>
        ...
        <li class="link-like"><a href="javascript:" class="ui-btn link_like" data-link="2" data-id="<?php echo $info['id'];?>"><i></i><span class="ui-btn-text"><?php if(!empty($info['islike'])) { ?>取消喜欢<?php } else { ?>喜欢<?php } ?>
<b>(<?php echo $info['likeNum'];?>)</b></span></a></li>
        <li class="link-comment" onclick="$('#report_area').focus();" ><a href="###" class="ui-btn"><i></i><span class="ui-btn-text">评论<b>(<?php echo $info['commentnum'];?>)</b></span></a></li>
        <li class="link-browse"><a href="#" class="ui-btn"><i></i><span class="ui-btn-text">浏览<b>(<?php echo $info['shownum'];?>)</b></span></a></li>
      </ul>
    </div>
  </div>
</div>
<?php
$main = substr($GLOBALS['cfg_basehost'],7);
?>
<script>
  //  设置 页面的  划词 和图片分享
  /*
  ...
  window._bd_share_config={
    common:{
               ...
               bdDesc : "这游记写得太棒了！",
               bdUrl  : "aitto.net/notes/show_<?php echo $info['id'];?>.html",
...
<div class="container">
  <section>
    <div class="ctd-head-travelNotes"><img src="<?php echo $info['litpic'];?>" width="100%" height="100%">
      <div class="ctd-mask"></div>
      <div class="ctd-info">
        <div class="user-img"><img src="<?php if(!empty($member['litpic'])) { ?><?php echo $member['litpic'];?><?php } else { ?><?php echo $info['litpic'];?><?php } ?>
" ></div>
        <div class="user-info">
          <h2><?php echo $info['title'];?></h2>
          <div class="user-bot">
            <div class="user-l"><span class="user-data"><?php echo date("j/M.Y",$info['addtime']);?></span><span class="user-name"><?php if(empty($member['nickname'])) { ?>匿名<?php } else { ?><?php echo $member['nickname'];?><?php } ?>
</span><span class="user-lv">LV <?php echo $info['greate'];?></span><span class="user-num"><?php echo $info['noteNum'];?>篇游记</span></div>
            <div class="user-r">
              <span class="link-box">
<!--                <a class="link-share bds_more" href="javascript:;" data-cmd="more"><i></i>分享 </a>-->
...
<span><?php echo $info['likeNum'];?></span> </a>
                <a class="link-comment" href="javascript:;" onclick="$('#report_area').focus();"  title="我要评论" ><i></i>评论<span><?php echo $info['commentnum'];?></span></a>
      ...
      <div class="ctd-Travel-nav">
        <div class="sideToolbar">
          <div class="sideCatalog">
            <div class="ebn-button-box" style="display: block;"> <a class="ebn-button ebn-button-up sideCatalog-up-disable" target="_self" href="javascript:;"></a> <a class="ebn-button ebn-button-down sideCatalog-down-enable" target="_self" href="javascript:;"></a> </div>
      ...
      <div class="Travel-article">
        <div class="ctd-content">
          <div class="ebma-chapter ebma-preface"> <a class="ebma-anchor0" href="#" id="p0"></a>
            ...
            <ul class="Survey">
              <li class="time"><i></i><span>目的地： <?php echo $info['mdd'];?></span></li>
              <?php $day = array(1=>"1天",2=>"2天",3=>"3天",4=>"4~7天",5=>"7天以上");?>
              <?php $way = array(1=>"跟团",2=>"自驾",3=>"自由行");?>
              <li class="day"><i></i><span>出行天数：  <?php echo $day[$info['travel_day']];?></span></li>
              <li class="cost"><i></i><span>人均花费：<?php echo $info['travel_consume'];?></span></li>
              <li class="people"><i></i><span>出行人物： <?php echo $info['travel_men'];?></span></li>
              <li class="jingqu"><i></i><span>出行方式： <?php echo $way[$info['travel_way']];?></span> </li>
              <li class="day"><i></i><span>出发日期：<?php echo $info['travel_date'];?></span></li>
              <div class="clearfix"></div>
            </ul>
          </div>
          <?php //var_dump($notelist['0']['child'])?>
          <?php $n=1; if(is_array($notelist)) { foreach($notelist as $k => $m) { ?>
          <div class="ebma-chapter chapter-main"> <a class="ebma-anchor0" href="#" id="p<?php echo $k+1;?>"></a>
            <h4 class="CatalogHd-main"><i></i><span class="head-con"><?php echo $m['title_s'];?></span></h4>
          </div>
          <div><p>
              <?php if($m['notes_id']>167) { ?>
              <?php echo htmlspecialchars_decode($m['content'],ENT_QUOTES);?>
              <?php } else { ?>
              <?php echo $m['content'];?>
              <?php } ?>
              </p>
          ...
          <div class="ebma-chapter chapter-child"> <a class="ebma-anchor1" href="#" id="p<?php echo $k+1;?>-<?php echo $ke;?>"></a>
            <h4 class="CatalogHd-child"><span class="head-con"><?php echo $j['title_s'];?></span></h4>
            <div class="ebma-con">
              <p>
                <?php if($m['notes_id']>167) { ?>
                <?php echo htmlspecialchars_decode($j['content'],ENT_QUOTES);?>
                <?php } else { ?>
                <?php echo $j['content'];?>
                <?php } ?>
              </p>
        ...
        <div class="ctd-comment">
          ...
          <div class="ebmc-content">
            ...
            <div class="nchcomments-box"> <a title="" class="nchimg"><img src="<?php if($user['litpic']) { ?><?php echo $user['litpic'];?><?php } else { ?><?php echo Common::nopic();?><?php } ?>
"></a>
              <div class="nchtextarea right"> <i class="tip-icon--left-sangle"></i>
                <textarea class="ebmc-textarea" cols="30" rows="10" placeholder="喜欢这篇游记吗？说点什么吧..." id="report_area" onkeyup="zifu(this)"></textarea>
      ...
      <div class="Travel-sider">
        <div class="notes-box">
          <div class="side-box side-About">
            <div class="wy-notice-img"><i></i><span><?php echo $info['allnotes'];?></span>篇游记</div>
            <div class="wy-notice-btns">
              <a href="/notes/WRITE" class="Publish-Travels" rel="nofollow" target="-blank"><i class="icon icon-publish"></i>发游记</a>
              <a href="javascript:"  rel="nofollow" id="com"><i class="icon icon-comments"></i>写点评</a> </div>
          </div>
          <div class="side-box sid-Recommend">
            <h4>热门游记推荐</h4>
            <ul>
              <?php $n=1; if(is_array($hot_notes)) { foreach($hot_notes as $hotR) { ?>
              <?php //var_dump($hotR)?>
              <li>
                <div class="note-img"><img src="<?php echo Common::img($hotR['litpic']);?>"/></div>
                <div class="note-con">
                  <h5 title="<?php echo $hotR['title'];?>" style="overflow: hidden"><?php echo $hotR['title'];?></h5>
                  <div class="note-name"><span><?php echo $hotR['member']['nickname'];?>&nbsp;&nbsp;&nbsp;&nbsp;</span><?php echo Common::mydate('Y-m-d',$hotR['addtime']);?> </div>
                </div>
              </li>
              <?php $n++;}unset($n); } ?>
            </ul>
...
<input type="hidden" id="user" value="<?php echo $user['mid'];?>"/>
<!----分页----->
