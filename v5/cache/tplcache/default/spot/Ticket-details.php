...
<title><?php echo $seoinfo['seotitle'];?>-<?php echo $GLOBALS['cfg_webname'];?></title>
  ...
  <?php echo Common::js('common/jquery-1.11.3.min.js,common/bootsAitto-min.js,myjs/spot/bootstrap-Ticket.js');?>
...
<div class="BreadCrumb Color-fff">
  <ul>
    <li><a href="<?php echo $cmsurl;?>"><?php echo $GLOBALS['cfg_indexname'];?></a><i class="icon-gt">&gt;</i></li>
    <li><a href="<?php echo $cmsurl;?>spots/"><?php echo $channelname;?><i class="arrow"></i></a><i class="icon-gt">&gt;</i></li>
    <?php $n=1; if(is_array($predest)) { foreach($predest as $dest) { ?>
    <li><a href="<?php echo $cmsurl;?>spots/<?php echo $dest['pinyin'];?>/"><?php echo $dest['kindname'];?><?php echo $channelname;?><i class="arrow"></i></a><i class="icon-gt"></i></li>
    <?php $n++;}unset($n); } ?>
    ...
    <div class="container"> 
      ...
      <div class="ctd-title">
        ...
        <div class="J-price">
          ...
          <div class="J-priceM"><?php if(!empty($info['price'])) { ?><i>¥</i><b><?php echo $info['price'];?></b><dfn>/人起</dfn><?php } else { ?>电询<?php } ?>
</div>
          ...
          <div class="stage">
            <div class="carousel carousel-stage">
              <ul>
                ...
                <li><img src="<?php echo Common::img($pic['0']);?>" alt=""> </li>
                <?php $n++;}unset($n); } ?>
              </ul>
            </div>
          </div>
          ...
          <a href="#" class="prev prev-navigation"></a> <a href="#" class="next next-navigation"></a>
            <div class="carousel carousel-navigation">
              <ul  style="margin-left: 80px;">
                <?php $n=1; if(is_array($info['piclist'])) { foreach($info['piclist'] as $pic) { ?>
<!--                <li><img src="<?php echo Common::img($pic['0'],90,59);?>" width="86" height="60" alt=""></li>-->
                <li><img src="<?php echo Common::img($pic['0']);?>" width="86" height="60" alt=""></li>
                <?php $n++;}unset($n); } ?>
              </ul>
            </div>
          </div>
      ...
      <div class="ctd-ticket-show-txt">
        ...
        <div class="ticket-hd">
          ...
          <div class="int"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo Common::cutstr_html($info['content'],300);?>… </div>
          <div class="way"> <i></i>
            <div class="citytit"><?php echo $info['address'];?></div>
          </div>
          <div class="time"> <i></i>
            <div class="citytit"><?php echo $extendinfo['e_business_time'];?></div>
          </div>
          <div class="tel"> <i></i>
            <div class="citytel"><?php echo $extendinfo['e_tel'];?></div>
          </div>
          <div class="preview-a">
            <div class="a-sales">已售<?php echo $info['sellnum'];?></div>
            <div class="a-score"><?php echo $info['satisfyscore'];?>分</div>
            <?php if($userip!=0) { ?>
            <div class="a-judge a_judges" data-id="0" style="background-position:25px -200px "><span><?php echo $info['recommendnum'];?></span>人推荐</div>
            <?php } else { ?>
            <div class="a-judge a_judges" data-id="0"><span><?php echo $info['recommendnum'];?></span>人推荐</div>
            <?php } ?>
          </div>
        </div>
      </div>
...
<!--          <li><a href="javascript:void(0)">酒店</a></li>-->
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tbody>
          ...
          <?php require_once ("/home/wwwroot/www.aitto.net/taglib/spot.php");$spot_tag = new Taglib_Spot();if (method_exists($spot_tag, 'suit_type')) {$typelist = $spot_tag->suit_type(array('action'=>'suit_type','row'=>'8','productid'=>$info['id'],'return'=>'typelist',));}?>
          <?php $n=1; if(is_array($typelist)) { foreach($typelist as $type) { ?>
          <?php require_once ("/home/wwwroot/www.aitto.net/taglib/spot.php");$spot_tag = new Taglib_Spot();if (method_exists($spot_tag, 'suit_by_type')) {$suitlist = $spot_tag->suit_by_type(array('action'=>'suit_by_type','row'=>'10','productid'=>$info['id'],'suittypeid'=>$type['id'],'return'=>'suitlist',));}?>
          <?php// var_dump($suitlist);?>
          <?php $n=1; if(is_array($suitlist)) { foreach($suitlist as $k => $suit) { ?>
          <tr>
            <td rowspan="2" valign="top"  class="tab-peo"><b><?php if($k>0) { ?><?php } else { ?><?php echo $type['title'];?><?php } ?>
</b></td>
            <td><div class="text-left"><?php echo $suit['title'];?> <a href="javascript:;" class="TableDetail-btn" ><i></i></a> </div></td>
            <td><?php echo $extendinfo['e_Advance_buy_time'];?></td>
            ...
            <td><div class="booking-but"><a class="booking-btn" href="javascript:;" data-suitid="<?php echo $suit['id'];?>"><?php if($suit['paytype']==4) { ?>景区支付<?php } else { ?>在线支付<?php } ?>
</a></div></td>
          </tr>
          <?php require_once ("/home/wwwroot/www.aitto.net/taglib/detailcontent.php");$detailcontent_tag = new Taglib_Detailcontent();if (method_exists($detailcontent_tag, 'get_content')) {$spotcontent = $detailcontent_tag->get_content(array('action'=>'get_content','pc'=>'1','typeid'=>$typeid,'productinfo'=>$info,'return'=>'spotcontent',));}?>
        ...
        <div class="ctdF-Case J-relative">
          <div class="ctdF-nav" >
            <ul data-toggle="J-tabs">
              ...
              <li><a href="#p3">用户点评(<?php echo $info['commentnum'];?>)</a></li>
            </ul>
            <?php if(!empty($info['price'])) { ?>
          ...
          <div class="ctd-chapter"><a class="ctd-anchor0" href="#" id="p3"></a>
            ...
            <div class="ctd-comment">
              <div class="ebmc-content">
                ...
                <div class="nchcomments-box nch-box"> <a title="" class="nchimg"><img src="<?php if($user['litpic']) { ?><?php echo $user['litpic'];?><?php } else { ?><?php echo Common::nopic();?><?php } ?>
"></a>
      ...
      <div class="ctd-side">
        <div class="side-box sid-Recommend">
          ...
          <ul>
            ...
            <li>
              <div class="note-img"><img src="<?php echo Common::img($spot['litpic']);?>"/><div class="note-price"><strong><?php if(!empty($spot['mdd'])) { ?><?php echo $spot['mdd'];?><?php } ?>
 </strong><?php if(!empty($spot['price'])) { ?><span>￥<em><?php echo $spot['price'];?></em></span><?php } else { ?><span>&nbsp;&nbsp;&nbsp;&nbsp;电询</span><?php } ?>
 </div></div>
              <div class="note-con">                
...
//        if (checkcode == '') {
//          layer.msg('<?php echo __("checkcode_empty");?>', {
//            icon: 5
//
//
//          })
...
<script type="text/javascript">
  ...
  var opts = {
    width : 300,     // 信息窗口宽度
    height: 125,     // 信息窗口高度
    title : "<a href='#' style='color:#4D4D4D;display:block;margin-bottom:5px' id='bt'><?php echo $info['title'];?></a>"  // 信息窗口标题
  }
  //var img  = document.getElementsByName("img");
  var mcon ="<span style='font-size:12px;color:#4D4D4D'>景区地址：<?php echo $info['address'];?></span><br/><span style='font-size:11.4px;color:#4D4D4D'>景区简介：<?php echo Common::cutstr_html($info['content'],42);?></span>";
...
//      var urls = "/spots/show_<?php echo $info['id'];?>.html?time_sort=desc";
//      //  }else{
...
//      var url ='/spots/show_<?php echo $info['id'];?>.html';
//      window.location.href =url;
//    });
//    //=================================06.29 fengishu end
