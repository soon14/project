...

    {
...

        {
            ...
            if(!empty($v['finaldestid'])) {
                ...
                $v['mdd'] = Model_Article::get_article_mdd($v['finaldestid']);
            }
...
//
//                break;
                $url .="&destpy=".$v."&priceid=".$p['priceid']."&sorttype=".$p['sorttype'];

                $url .= "&attrid=".$p['attrid']."&p=".$currentpage;
                break;
...
//                $url.=$p['attrid'].'-'.$currentpage;
                ...
                $url .="&destpy=".$p['destpy']."&priceid=".$v."&sorttype=".$p['sorttype'];
...
//                $url.=$p['attrid'].'-'.$currentpage;
                ...
                $url .="&destpy=".$p['destpy']."&priceid=".$p['priceid']."&sorttype=".$v;
...
//                $url.=$attr_list.'-'.$currentpage;
                ...
                $url .="&destpy=".$p['destpy']."&priceid=".$p['priceid']."&sorttype=".$p['sorttype'];

                $url .= "&attrid=".$attr_list."&p=".$currentpage;
...

    {
...

        {
            ...
            $url = self::get_search_mddurl($p['destpy'],'spot','all','destpy',$p);
...

        {
            ...
            $url = self::get_search_mddurl($p['destpy'],'spot','0','priceid',$p);
...

        {
...

            {
               ...
               // $url = $GLOBALS['cfg_basehost'].'/spots/';
                $url = $GLOBALS['cfg_basehost'].'/'.$p['destpy'].'/?param=spot';
