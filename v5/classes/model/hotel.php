...
class Model_Hotel extends ORM
{
    ...
    * */
    public static function get_minprice($hotelid,$roomid='')
    {
        ...
        $sql = "SELECT MIN(price) AS price FROM `sline_hotel_room_price` WHERE hotelid='$hotelid' AND price!=0 AND day>=UNIX_TIMESTAMP() {$where}";
        $row = DB::query(1,$sql)->execute()->as_array();
...
//                $url.=$p['displaytype'].'-'.$attr_list.'-'.$currentpage;
    ...
    public static function get_selected_item($p)
    {
        ...
        if($p['rankid']!=0)
        {
            ...
            $temp['url'] = $url;
            $temp['itemname'] = ORM::factory('hotel_rank', $p['rankid'])->get('hotelrank');
            $out[] = $temp;
        ...
        if($p['priceid']!=0)
        {
            ...
            $temp['url'] = $url;
            $ar = ORM::factory('hotel_pricelist',$p['priceid'])->as_array();
            $lowerprice = $ar['min'];
            $highprice = $ar['max'];
        ...
        if($p['attrid']!=0)
        {
            ...
            foreach($attArr as $ar)
            {
                ...
                $orgArr = $attArr;
                $temp = array();
                $temp['itemname'] = ORM::factory('hotel_attr',$ar)->get('attrname');
    ...
    public static function get_selected_mdditem($p)
    {
        ...
        if($p['destpy']!='all')
        {
            ...
            $url = self::get_search_mddurl($p['destpy'],'hotel','all','destpy',$p);
        ...
        {
            $temp = array();
            $url = self::get_search_mddurl($p['destpy'],'hotel','0','rankid',$p);
            $temp['url'] = $url;
            $temp['attr'] = '星级';
            $temp['itemname'] = ORM::factory('hotel_rank', $p['rankid'])->get('hotelrank');
            $out[] = $temp;
        ...
        {
            $temp = array();
            $url = self::get_search_mddurl($p['destpy'],'hotel','0','priceid',$p);
            $temp['url'] = $url;
            $ar = ORM::factory('hotel_pricelist',$p['priceid'])->as_array();
            $lowerprice = $ar['min'];
            $highprice = $ar['max'];
        ...
        if($p['attrid']!=0)
        {
            ...
            foreach($attArr as $ar)
            {
                ...
                $temp = array();
                $temp['itemname'] = ORM::factory('hotel_attr',$ar)->get('attrname');
                $thisPid          = ORM::factory('hotel_attr',$ar)->get('pid');
                $temp['attr']     = ORM::factory('hotel_attr',$thisPid )->get('attrname');
                unset($orgArr[array_search($ar,$orgArr)]);
                if(!empty($attrid))
                {
                    $attrid = implode('_',$orgArr);
                ...
                $url = $GLOBALS['cfg_basehost'].'/'.$p['destpy'].'/?param=hotel';
//                $url.=$p['destpy'].'-'.$p['rankid'].'-'.$p['priceid'].'-'.$p['sorttype'].'-';
//                $url.=$p['displaytype'].'-'.$attrid.'-1';
    ...
    public static function gen_seotitle($param)
    {
        ...
        if(!empty($param['rankid']))
        {
            ...
            $arr[] = ORM::factory('hotel_rank', $param['rankid'])->get('hotelrank');
        }

        if(!empty($param['priceid']))
        {
            $priceArr= ORM::factory('hotel_pricelist',$param['priceid'])->as_array();
