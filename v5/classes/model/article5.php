...
class Model_Article extends ORM
{
     ...
     */
     public static function get_ar_attrid(){
         $sql  = 'SELECT * FROM `sline_article_attr` where isopen=1 ORDER by id desc ';
         $arr = DB::query(1, $sql)->execute()->as_array();
 ...
 */
     ...
     */
    public  static  function get_hot_article(){
        $sql = "SELECT * FROM `sline_article` where ishidden=0 ORDER by shownum DESC limit 0,3 ";
        $arr = DB::query(1,$sql)->execute()->as_array();
        foreach($arr as &$v)
        {
            $k               =array();
