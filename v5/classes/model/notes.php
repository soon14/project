...

{
        ...
        // $totalSql   =SELECT * FROM `sline_notes` WHERE is_draft=0 and status=1 and find_in_set($mid,is_like))"
...

         {
             if($gl=='') {
                 ...
                 $row['piclist']  = Product::pic_list($arr1[0]['piclist']);
    ...
    public  static function  get_noteContent($noteid){
        ...
        foreach ($notecontent as &$v){
            ...
            $con.=htmlspecialchars_decode($v['content'],ENT_QUOTES);
        }
       ...
       //   20170223  封冀蜀 将 大于 1  改为 大于0
          if($get['m_length']>0){
              ...
              for($i=0;$i<$get['m_length'];$i++){
                      ...
                      $title  = $_POST['A'.($i+3)];
                      $content= htmlspecialchars($_POST['A'.($i+3).'_con'],ENT_QUOTES);
                      $m_id   = "A".($i+3);
          ...
          if($get['z_length']>0){
              ...
              for($k=0;$k<$get['m_length'];$k++){
                  ...
                  for($j=0;$j<$zmulu;$j++){
                   ...
                   $content_z = htmlspecialchars($_POST[$m_id.'-'.($j+1).'_UP'],ENT_QUOTES);
   ...
   */
    public  static function update_mulu2($post=array(),$get=array(),$noteid){
        ...
        if($get['m_length']>0){
            ...
            for($i=0;$i<$get['m_length'];$i++){
                ...
                $content= htmlspecialchars($_POST['A'.($i+2).'_con'],ENT_QUOTES);
                $m_id   = "A".($i+2);
    ...
    public  function del_mainlu($noteid,$id){
        ...
        $sql  = " DELETE FROM `sline_notes_list` WHERE id =$id and notes_id=$noteid";

      //  Common::read_log($sql);

        DB::query(1,$sql)->execute();
        $sql  = " DELETE FROM `sline_notes_list` WHERE pid =$id  and notes_id=$noteid";
    ...
    */
    ...
    public static function search_result($params,$keyword,$currentpage,$pagesize='10')
    {
        ...
        $day = Common::remove_xss($params['day']);

        $way = Common::remove_xss($params['way']);
        ...
        if($destPy && $destPy!='all')
        {
            ...
            $where.= " AND a.finaldestid=$destId ";
        }
        if(!empty($day)){

            $where .= " and a.travel_day =$day ";
        }
        if(!empty($way)){

            $where .= " and a.travel_way =$way ";
        }
...
//                $where.= "AND a.title like '%$keyword%' or FIND_IN_SET('$keyword',a.keyword)  ";
    ...
    public static function get_search_url($v,$paramname,$p,$currentpage=1)
    {
        ...
        $url = $GLOBALS['cfg_basehost'].'/notes/';
        switch($paramname)
        {
            case "destpy":

                $url.=$v.'-'.$p['day'].'-'.$p['way'].'-'.$currentpage;
                break;

            case "day":

                $url.=$p['destpy'].'-'.$v.'-'.$p['way'].'-'.$currentpage;
                break;

            case "way":

                $url.=$p['destpy'].'-'.$p['day'].'-'.$v.'-'.$currentpage;
                break;
        }
        return $url;
    ...
    public static function get_search_mddurl($mdd,$param,$v,$paramname,$p,$currentpage=1)
    {
        ...
        switch($paramname)
        {
            case "destpy":
              ...
              //  $url.=$v.'-'.$p['day'].'-'.$p['way'].'-'.$currentpage;
                $url .="&destpy=".$v."&day=".$p['day']."&way=".$p['way']."&p=".$currentpage;
                break;

            case "day":

               // $url.=$p['destpy'].'-'.$v.'-'.$p['way'].'-'.$currentpage;
                $url .="&destpy=".$p['destpy']."&day=".$v."&way=".$p['way']."&p=".$currentpage;
                break;

            case "way":

                //$url.=$p['destpy'].'-'.$p['day'].'-'.$v.'-'.$currentpage;
                $url .="&destpy=".$p['destpy']."&day=".$p['day']."&way=".$v."&p=".$currentpage;
                break;
        }
        return $url;
    ...
    public static function get_selected_item($p)
    {
        ...
        //属性
        if($p['day']!=0)
        {  $day = array(1=>"1天",2=>"2天",3=>"3天",4=>"4~7天",5=>"7天以上",);
            $temp = array();
            $url = self::get_search_url(0,'day',$p);
            $temp['url'] = $url;
            $temp['attr'] = '出行天数';
            $temp['itemname'] = $day[$p['day']];
            $out[]=$temp;
        ...
        if($p['way']!=0)
        {
            $way = array(1=>"跟团",2=>"自驾",3=>"自由行");
            $temp = array();
            $url = self::get_search_url(0,'way',$p);
            $temp['url'] = $url;
            $temp['attr'] = '出行方式';
            $temp['itemname'] = $way[$p['way']];
            $out[]=$temp;
    ...
    public static function get_selected_mdditem($p)
    {
        $p['displaytype'] = 0;
        $out = array();
        //目的地$mdd,$param,$v,$paramname,$p,$currentpage=1
        if($p['destpy']!='all')
        {
            $temp = array();
            $url = self::get_search_mddurl($p['destpy'],'note',$p['destpy'],'destpy',$p);
            $temp['url'] = $url;
            $temp['attr'] = '目的地';
            $temp['itemname'] = ORM::factory('destinations')->where("pinyin='".$p['destpy']."'")->find()->get('kindname');
            $out[]=$temp;
        ...
        {  $day = array(1=>"1天",2=>"2天",3=>"3天",4=>"4~7天",5=>"7天以上",);
            $temp = array();
            $url = self::get_search_mddurl($p['destpy'],'note',0,'day',$p);
            $temp['url'] = $url;
            $temp['attr'] = '出行天数';
            $temp['itemname'] = $day[$p['day']];
            $out[]=$temp;
        ...
        if($p['way']!=0)
        {
            $way = array(1=>"跟团",2=>"自驾",3=>"自由行");
            $temp = array();
            $url = self::get_search_mddurl($p['destpy'],'note',0,'way',$p);
            $temp['url'] = $url;
            $temp['attr'] = '出行方式';
            $temp['itemname'] = $way[$p['way']];
            $out[]=$temp;
