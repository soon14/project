...

{
...

    {
        ...
        $file  = fopen('fengdd.txt','a+');

        fwrite($file,time().":".$flag."\r\n");
        fclose($file);
...

    {
        ...
        $token = Arr::get($_POST,'token');//MD5（time（））
...

    {
...

        {
            ...
            $msgInfo = Product::get_define_msg(0,0,'reg_findpwd');
            ...
            {
               //  $flag 返回是一个null 值   现在先把下面一句隐藏  07.011 封冀蜀
              //  return array('status'=>false,'msg'=>'验证码发送失败，请重试'.$flag->Message);
                ...
                Common::session('mobilecode_'.$mobile,$code);
               // $c=Common::session('mobilecode_'.$mobile);
                return array('status'=>true,'msg'=>'验证码发送成功');
...

    {
        ...
        $emailInfo=Product::get_email_msg_config('reg_findpwd');
