...
<title>{$info['title']}-{$webname}</title>
  ...
  {Common::css("mycss/strategy/TravelNotes.css")}
...
<div class="travelFloat-nav">
  <div class="nav-warp">
    <div class="nav-title"> <span class="home"><a href="#"><em></em></a></span> <span class="nav-h2">{$info['title']}</span> </div>
    <div class="ui-navArea-btn">
      <ul>
        ...
        <li class="link-like"><a href="javascript:" class="ui-btn link_like" data-link="2" data-id="{$info['id']}"><i></i><span class="ui-btn-text">{if !empty($info['islike'])}取消喜欢{else}喜欢{/if}<b>({$info['likeNum']})</b></span></a></li>
        <li class="link-comment" onclick="$('#report_area').focus();" ><a href="###" class="ui-btn"><i></i><span class="ui-btn-text">评论<b>({$info['commentnum']})</b></span></a></li>
        <li class="link-browse"><a href="#" class="ui-btn"><i></i><span class="ui-btn-text">浏览<b>({$info['shownum']})</b></span></a></li>
      </ul>
    </div>
  </div>
</div>
...
<script>
  ...
  window._bd_share_config={
    common:{
               bdText : "{$info['title']}",
               bdDesc : "这游记写得太棒了！",
               bdUrl  : "aitto.net/notes/show_{$info['id']}.html",
               bdPic  : "aitto.net{$info['litpic']}",
               bdMini :2,
...
<div class="container">
  <section>
    <div class="ctd-head-travelNotes"><img src="{$info['litpic']}" width="100%" height="100%">
      <div class="ctd-mask"></div>
      <div class="ctd-info">
        <div class="user-img"><img src="{if !empty($member['litpic'])}{$member['litpic']}{else}{$info['litpic']}{/if}" ></div>
        <div class="user-info">
          <h2>{$info['title']}</h2>
          <div class="user-bot">
            <div class="user-l"><span class="user-data">{date("j/M.Y",$info['addtime'])}</span><span class="user-name">{if empty($member['nickname'])}匿名{else}{$member['nickname']}{/if}</span><span class="user-lv">LV {$info['greate']}</span><span class="user-num">{$info['noteNum']}篇游记</span></div>
            <div class="user-r">
              <span class="link-box">
<!--                <a class="link-share bds_more" href="javascript:;" data-cmd="more"><i></i>分享 </a>-->
                <a class="link-like link_like" href="javascript:;"  data-id="{$info['id']}"  title="喜欢就点击一下"> <i></i>{if !empty($info['islike'])}取消喜欢{else}喜欢{/if}<span>{$info['likeNum']}</span> </a>
                <a class="link-comment" href="javascript:;" onclick="$('#report_area').focus();"  title="我要评论" ><i></i>评论<span>{$info['commentnum']}</span></a>
                <a class="link-browse" href="javascript:;"> <i></i> 浏览 <span>{$info['shownum']}</span> </a> </span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="Travel-article">
        <div class="ctd-content">
          <div class="ebma-chapter ebma-preface"> <a class="ebma-anchor0" href="#" id="p0"></a>
            ...
            <ul class="Survey">
              <li class="time"><i></i><span>目的地： {$info['mdd']}</span></li>
              <?php $day = array(1=>"1天",2=>"2天",3=>"3天",4=>"4~7天",5=>"7天以上");?>
              <?php $way = array(1=>"跟团",2=>"自驾",3=>"自由行");?>

              <li class="day"><i></i><span>出行天数：  {$day[$info['travel_day']]}</span></li>
              <li class="cost"><i></i><span>人均花费：{$info['travel_consume']}</span></li>
              <li class="people"><i></i><span>出行人物： {$info['travel_men']}</span></li>
              <li class="jingqu"><i></i><span>出行方式： {$way[$info['travel_way']]}</span> </li>
              <li class="day"><i></i><span>出发日期：{$info['travel_date']}</span></li>
              <div class="clearfix"></div>
            </ul>
          </div>
          <?php //var_dump($notelist[0]['child'])?>
          {loop $notelist $k $m}
          <div class="ebma-chapter chapter-main"> <a class="ebma-anchor0" href="#" id="p{$k+1}"></a>
            <h4 class="CatalogHd-main"><i></i><span class="head-con">{$m['title_s']}</span></h4>
          </div>
          <div><p>
              {if $m['notes_id']>167}
              {htmlspecialchars_decode($m['content'],ENT_QUOTES)}
              {else}
              {$m['content']}
              {/if}
              </p>
          ...
          {if !empty($m['child'])}
          {loop $m['child'] $ke $j}
          <div class="ebma-chapter chapter-child"> <a class="ebma-anchor1" href="#" id="p{$k+1}-{$ke}"></a>
            <h4 class="CatalogHd-child"><span class="head-con">{$j['title_s']}</span></h4>
            <div class="ebma-con">
              <p>
                {if $m['notes_id']>167}
                {htmlspecialchars_decode($j['content'],ENT_QUOTES)}
                {else}
                {$j['content']}
                {/if}
              </p>
        ...
        <div class="ctd-comment">
          ...
          <div class="ebmc-content">
            ...
            <div class="nchcomments-box"> <a title="" class="nchimg"><img src="{if $user['litpic']}{$user['litpic']}{else}{Common::nopic()}{/if}"></a>
      ...
      <div class="Travel-sider">
        <div class="notes-box">
          <div class="side-box side-About">
            <div class="wy-notice-img"><i></i><span>{$info['allnotes']}</span>篇游记</div>
          ...
          <div class="side-box sid-Recommend">
            ...
            <ul>
              ...
              <?php //var_dump($hotR)?>
              <li>
                <div class="note-img"><img src="{Common::img($hotR['litpic'])}"/></div>
                <div class="note-con">
                  <h5 title="{$hotR['title']}" style="overflow: hidden">{$hotR['title']}</h5>
                  <div class="note-name"><span>{$hotR['member']['nickname']}&nbsp;&nbsp;&nbsp;&nbsp;</span>{Common::mydate('Y-m-d',$hotR['addtime'])} </div>
                </div>
              </li>
              {/loop}
...
<input type="hidden" id="user" value="{$user['mid']}"/>
<!----分页----->
