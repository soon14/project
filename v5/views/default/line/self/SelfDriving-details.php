...
<title>{$seoinfo['seotitle']}-{$GLOBALS['cfg_webname']}</title>
  ...
  {Common::js('common/jquery-1.11.3.min.js,common/bootsAitto-min.js,myjs/selfDriving/bootstrap-SelfDriving.js')}
<style>
  .box2{
    display: none;
    width: 100%;
...
<section> 
  ...
  <div  class="BreadCrumb Color-fff">
    <ul >
      ...
      <li><a href="/lines/self">{$channelname}<i class="icon-gt">&gt;</i></a></li>
      {loop $predest $dest}
      <li class="current"><a href="/lines/self/{$dest['pinyin']}/">{$dest['kindname']}</a></li>
      {/loop}
    </ul>
...
<section>
  <div class="ctd-head-SelfDriving">
    ...
    <div class="SelfDriving-Head">
      <h1><em></em>{$info['title']}</h1>
      <div class="d-nav">
        <ul>
          <li class="trip-ico"><i></i><span>自驾日期：{$info['linedate']['starttime']}~{$info['linedate']['endtime']}</span></li>

        </ul>
      </div>
      <div class="d-price">
        <div class="d-priceA"></div>
        <div class="d-priceM"><i>¥</i><b>{$info['storeprice']}</b><dfn>/人起</dfn></div>
...
<?php //var_dump($info['piclist'])?>
          <tr>
            <td width="64%" rowspan="2" valign="top"><div class="bigimg"><img src="{$info['piclist'][0][0]}"></div></td>
            <td width="18%"><div class="smallimg" style="margin-bottom:6px;"><img src="{$info['piclist'][1][0]}"></div></td>
            <td width="18%"><div class="smallimg" style="margin-bottom:6px;"><img src="{$info['piclist'][2][0]}"></div></td>
          </tr>
          <tr>
            <td><div class="smallimg"style="margin-top:6px;"><img src="{$info['piclist'][3][0]}"></div></td>
            <td><div class="smallimg"style="margin-top:6px;"><img src="{$info['piclist'][4][0]}"></div></td>
          </tr>
        </table>
      </div>
  <section> 
    ...
    <div  class="ctd-SelfDrive-booking" id="orderID" >
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tbody>
          ...
          {st:line action="suit" row="5" productid="$info['id']" }
          <?php //var_dump($data);?>
          {loop $data  $type}
          ...
          <tr>
            ...
            <td>
              <div class="text-left">{$type['title']}
                <a href="javascript:;" class="TableDetail-btn" ><i></i></a>
              </div>
            </td>
            <td>{Currency_Tool::symbol()}{$lastoffer['adultprice']}</td>
            <td>{if empty($lastoffer['childprice'])}暂无{else}{Currency_Tool::symbol()}{$lastoffer['childprice']}{/if}</td>
            <td>{if empty($lastoffer['oldprice'])}暂无{else}{Currency_Tool::symbol()}{$lastoffer['oldprice']}{/if}</td>
            <td><div class="booking-but"><a class="booking-btn one" href="javascript:;" data-suitid="{$info['suitid']}" data-lineid="{$info['id']}" data-endtime="{$info['linedate']['endtime']}"  data-chatime="{$info['cha_time']}" >在线预定</a></div></td>
          </tr>
    ...
    <div class="ctd-SelfDrive-body"> 
        ...
        <div class="ctdF-Case J-relative">
          <div class="ctdF-nav" >
            <ul data-toggle="J-tabs">
              ...
              <li><a href="#p7">用户点评{if !empty($info['commentnum'])}({$info['commentnum']}){/if}</a></li>
            </ul>
      ...
      <div class="ctd-main"> 
        ...
        <div class="ctd-content">
          ...
          ?>
          <div class="ctd-chapter"><a class="ctd-anchor0" id="p{$k}"></a>
            <div class="SelfDriveM-Tit"> <span> <i class="detail-icon-2">{$rows['chinesename']} </i> <i class="detail-icon-t"> </i></span></div>
            <div class="ctd-con">
              {$rows['content']}
            </div>
          </div>
              ...
              <div class="ebmc-content">
                ...
                <div class="nchcomments-box nch-box">
                  <a title="" class="nchimg">
                    <img src="{if $userinfo['litpic']} $userinfo['litpic']{else}{Common::nopic()}{/if}">
                  </a>
        ...
        <div class="side-box sid-Recommend">
          ...
          <ul>
            ...
            <a href="/lines/self_show_{$h['aid']}.html">
              <li>
                <div class="note-img"><img src="{Common::img($h['piclist'][1][0])}"/><div class="note-price"><strong>{if !empty($h['startcity'])}出发地:{$h['startcity']}{/if}</strong><span>￥<em>{$h['storeprice']}</em></span> </div></div>
                <div class="note-con">
                  <h5>{$h['title']}</h5>
                </div>
              </li>
