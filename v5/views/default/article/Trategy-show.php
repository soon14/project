...
<title>{$seoinfo['seotitle']}-{$GLOBALS['cfg_webname']}</title>
  ...
  {Common::css("mycss/strategy/Trategy.css")}
  {Common::js('common/jquery-1.11.3.min.js,common/bootsAitto-min.js')}
  {Common::js('strategy/Trategy.js')}
...
<section> 
  ...
  <div  class="BreadCrumb">
    <ul >
      <li><a href="/">{$GLOBALS['cfg_indexname']}<i class="icon-gt">&gt;</i></a></li>
      <li><a href="/raiders">{$channelname}<i class="icon-gt">&gt;</i></a></li>
  ...
  <div class="nav-warp">
    ...
    <div class="ui-navArea-btn ">
      <ul>
        ...
        <li class="link-like"><a href="javascript:" class="ui-btn link_like" data-link="2" data-id="{$info['id']}"><i></i><span class="ui-btn-text">喜欢<b>({$info['likeNum']})</b></span></a></li>
        <li class="link-comment" onclick="foc()"><a href="javascript:void (0)" class="ui-btn"><i></i><span class="ui-btn-text" >评论<b>({$info['commentnum']})</b></span></a></li>
        <li class="link-browse"><a href="javascript;" class="ui-btn"><i></i><span class="ui-btn-text">浏览<b>({$info['shownum']})</b></span></a></li>
      </ul>
    </div>
  </div>
</div>
<script>
  ...
  window._bd_share_config={
    common:{
      bdText : "{$info['title']}",
      bdDesc : "这攻略写得太棒了！",
      bdUrl  : "aitto.net/raiders/show_{$info['aid']}.html",
...
<div class="container">
  <section>
    <div class="ctd-head-TrategyNotes"><img src="{$info['litpic']}" width="100%">
      <div class="ctd-mask"></div>
      <div class="ctd-info">
        <div class="user-img"><img src="{if !empty($info['authorhead'])}{$info['authorhead']}{else}{$info['litpic']}{/if}" ></div>
        <div class="user-info">
          <h2>{$info['title']}</h2>
          <div class="user-bot">
            <div class="user-l">
              <span class="user-data">{date("j/M.Y",$info['addtime'])}</span>
              <span class="user-name">{if empty($info['author'])}匿名{else}{$info['author']}{/if}</span>
              <span class="user-lv">LV {$info['author_lv']}</span><span class="user-num">{$info['all_article']}篇攻略</span>
            </div>
            <div class="user-r">
              <span class="link-box">
<!--                <a class="link-share bds_more" href="javascript:;" > <i></i>分享 </a>-->
                ...
                <a class="link-comment" href="javascript:;"  title="我要评论" onclick="foc()"><i></i>评论<span>{$info['commentnum']}</span></a>
      ...
      <div class="ctd-Travel-article">
        <div class="ctd-content">
          <div class="ebma-chapter ebma-preface"> <a class="ebma-anchor0" href="#" id="p0"></a>
            ...
            <ul class="Survey">
              <li class="time"><i></i><span>目的地： {$extendinfo['e_travel_mdd']}</span></li>
              <li class="day"><i></i><span>出行天数： {$extendinfo['e_travel_day']}</span></li>
              <li class="cost"><i></i><span>人均花费： {$extendinfo['e_travel_sonsume']}</span></li>
              <li class="people"><i></i><span>最佳出游时间：{$extendinfo['e_travel_time']}</span></li>
              <li class="jingqu"><i></i><span>景点：{$extendinfo['e_travel_spot']}</span> </li>
              <div class="clearfix"></div>
            </ul>
          </div>
          ...
          <div class="ebma-chapter chapter-main"> <a class="ebma-anchor0" href="#" id="p2"></a>
            ...
            <div class="ebma-con">
              {$extendinfo['e_travel_traffic']}
            </div>
          </div>

          {loop $mulu $k $m}
          <div class="ebma-chapter chapter-main"> <a class="ebma-anchor0" href="#" id="p{$k+4}"></a>
            <h4 class="CatalogHd-main"><i></i><span class="head-con">{$m['title']}</span></h4>
          </div>
          <div>{$m['content']} </div>
          {if !empty($m['child'])}
          {loop $m['child'] $ke $j}
          <div class="ebma-chapter chapter-child"> <a class="ebma-anchor1" href="#" id="p{$k+4}-{$ke}"></a>
            <h4 class="CatalogHd-child"><span class="head-con">{$j['title']}</span></h4>
            <div class="ebma-con">
              {$j['content']}
            </div>
          </div>
          ...
          <div class="ebmc-content">
            ...
            <div class="nchcomments-box">
              <a title="" class="nchimg">
                <img src="{if !empty($userinfo['litpic'])}{$userinfo['litpic']}{else}{Common::nopic()}{/if}">
              </a>
      ...
      <div class="ctd-Travel-sider">
        <div class="notes-box">
          <div class="side-box side-About">
            <div class="wy-notice-img"><i></i><span>{$info['allraider']}</span>篇攻略</div>
          ...
          <div class="side-box sid-Recommend">
            ...
            <ul>
              ...
              <li>
                ...
                <div class="note-con">
                  <h5>{$hotR['title']}</h5>
                  <div class="note-name"><span>{$hotR['author']}</span>&nbsp;&nbsp;{$hotR['addtime']} </div>
                </div>
                </a>
              </li>
              {/loop}
