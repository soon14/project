...

    {
...

        {
            ...
            case 'attr':
                ...
                break;
            case 'sellmore':
                $list = self::get_spot_by_bookcount($offset,$row);
                break;
        ...
        //对获取的数据进行初始化处理
       $list   = Model_Spot::format_spot_row($list);
//        foreach($list as &$v)
//
//        {
//
...
//        }
...

    {
        ...
        $suit = ORM::factory('spot_ticket')
            ...
            ->where("spotid=:productid and tickettypeid=:tickettypeid order by displayorder asc ")
...

        {
           ...
           // $v['ourprice'] = Currency_Tool::price($v['ourprice']); fengjishu 07.30



            $priceArr = Model_Spot::get_minprice($v['spotid']);
...

    {
        ...
        $arr = self::execute($sql);
        foreach($arr as &$v){
            $sql  =  "select kindname from `sline_destinations` where  id =$v[finaldestid] ";
           $v['finaldest']= DB::query(1,$sql)->execute()->get('kindname');
        }
...

    {
        ...
        foreach($arr as &$v){
            $v['mdd']  =ORM::factory('destinations',$v['id']) ->get('kindname');

            $price = Model_Spot::get_minprice($v['id']);

            $v['price'] = $price['price'];

            $v['sellprice'] = $v['sellprice'];
 ...
 * */
    ...
    private static function get_spot_by_attrid($offset,$row,$attrid)
    {
        ...
        $sql.=" AND FIND_IN_SET($attrid,a.attrid) ORDER BY a.modtime desc,a.addtime DESC ";
        $sql.= "LIMIT {$offset},{$row}";
        $arr = self::execute($sql);
        foreach($arr as $k =>&$v){
            $price = Model_Spot::get_minprice($v['id']);
            ...
            $v['sellprice'] = $price['sellprice'];

            $v['mdd']    = ORM::factory('destinations',$v['finaldestid'])->get('kindname');
        }
        return $arr;
    }
    /**
     ...
     * 根据预定的数量  确认   热卖的票务
     */
    private static function get_spot_by_bookcount($offset,$row)
    {
        $sql = "SELECT ".self::$basefield." FROM `sline_spot` a ";
        $sql.= "WHERE a.ishidden=0 ";
        $sql.=" and a.isrecommend=1 ";//1位推荐  封冀蜀 07.06
        $sql.="  ORDER BY a.modtime desc,a.addtime DESC,CONVERT(a.bookcount,SIGNED) desc ";
        $sql.= "LIMIT {$offset},{$row}";
        $arr = self::execute($sql);
        foreach($arr as $k =>&$v){
            $price = Model_Spot::get_minprice($v['id']);
