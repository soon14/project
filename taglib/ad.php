...

{
   ...
   */
...

    {
        ...
        //=========================
        $admdd  = unserialize($ad['admdd']);

        $adtime   =unserialize($ad['adtime']);
...

        {
            ...
            $ad['aditems'][$i] = array('adsrc' => $adsrc[$i], 'adlink' => $adlink[$i], 'adname' => $adname[$i], 'adorder' => $adorder[$i],'admdd'=>$admdd[$i],'adtime'=>$adtime[$i]);
...

    {
...

        {
            ...
            $sql = "SELECT * FROM sline_advertise_5x WHERE is_show='1' AND (CONCAT(prefix,'_',number) = '$name' {$pc_where} or custom_label='{$custom}')  AND is_show='1' AND webid='$sys_webid' ";
