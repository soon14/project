    ...
    return {
        init: function() {
            ...
            e.run(function() {
                ...
                e.canvas.fillStyle("#FFF").fillScreen().fillStyle("#000").drawString(l.moduleName, 0, u + 110, e.graphics.VCENTER, !1, null, null, "50px 微软雅黑").drawImage("btns1", 0, 106, 480, 7, i - 480 >> 1, u + 140, 480, 7).drawImage("btns1", 0, 106, 480, 7, i - 480 >> 1, u + 555, 480, 7).fillStyle("#000").drawString(a(l.time, "秒"), 0, u + 340, e.graphics.VCENTER, !1, null, null, "60px 微软雅黑").fillStyle("#000").drawString("最佳:" + a(l.bestTime, "秒"), 0, u + 400, e.graphics.VCENTER, !1, null, null, "30px 微软雅黑"),
                l.time > l.bestTime && e.canvas.fillStyle("#F00").drawString("新纪录", 0, u + 240, e.graphics.VCENTER, !1, null, null, "50px 微软雅黑"),
            ...
            c = function(t) {//开始游戏
                ...
                $.ajax({
                    type:"POST",
                    url:'/phone/activity/bggame',
                    data:{openid:openid},
                    dataType:'json',
                    success:function(data){
                        //&&
                        if(data.status==1111){//未关注
                        ...
                        }else if(data.status==1115){// 关注 未分享
                            var thisObjID  ="#DialogDiv5";
                            $(thisObjID).addClass('in').children().children().css({"padding-top":"30px",'height':'140px'}).children('.modal-body').css("font-size","30px").text(data.msg);
                            var thisObjcon = $(thisObjID).children(".modal-dialog");
                            thisObjcon.css({
                                'top': "35%",
                                'left': "10%"
                           ...
                           // setTimeout(function(){ $(thisObjID).removeClass('in')},1500);
                            ...
                            $("div[class=gz]").css("display","none");
                            $("div[class=top]").css("display","none");
            ...
            h = function() {
                ...
                //    $(thisObjID).addClass('in').children().children().children('.modal-body').text("强！你居然坚持了" + l.time / 1000 + "秒！");
                //    var thisObjcon = $(thisObjID).children(".modal-dialog");
                //    thisObjcon.css({
                //        'top': "35%",
                //        'left': "10%"
                ...
                var n = e.localStorage.getItem("NotDieAnyoneBestTime1" + l.module);
                n == null && (n = "0"),
                l.bestTime = parseInt(n),
                $.ajax({
                        type:'post',
                        url :'/phone/activity/updatetime',
                        data:{time: l.time,openid:$("#openid").val()},
                        dataType:'json',
                        success:function(data){
                           // alert(data.arrtime)
                            l.time > l.bestTime && e.localStorage.setItem("NotDieAnyoneBestTime1" + l.module, l.time.toString()),//更新成绩
                                dp_submitScore(l.module, l.time * (l.module - 1))
                        }
                })
                //l.time > l.bestTime && e.localStorage.setItem("NotDieAnyoneBestTime" + l.module, l.time.toString()),//更新成绩
                //dp_submitScore(l.module, l.time * (l.module - 1))
            }
        }
    }
