...
class Taglib_Ad
{
   ...
   */
    public static function getad($params)
    {
        ...
        $sql = "SELECT * FROM sline_advertise_5x WHERE ((CONCAT(prefix,'_',number) = '{$name}' {$pc_where}) or custom_label='{$custom}') AND is_show='1' ";
        if (!empty($destid))
        {
    ...
    public static function sortad($params)
    {
        ...
        if (!empty($tag))
        {
            ...
            $pc_where .= $bool ? " AND is_system='1'" : " AND is_system='0'";
            $name = substr($tag, 2);
            $sql = "SELECT * FROM sline_advertise_5x WHERE CONCAT(prefix,'_',number) = '$name' AND is_show='1' {$pc_where} or custom_label='{$custom}'";
            $ar = DB::query(1, $sql)->execute()->as_array();
            if (count($ar) <= 0)
            {
                return $ar;
