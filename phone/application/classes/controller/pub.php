...
class Controller_Pub extends Stourweb_Controller
{
     ...
     * 米淘头部
     * */
    public function action_header2()
    {

        $this->display('pub/header2');
    }

    /*
* 米淘头部
* */
    public function action_header3()
    {

        $this->display('pub/header3');
    }
 ...
 * 日历筛选
 * */
    public function action_calendar3()
    {
        $typeid = $this->params['typeid'];
        $suitid = $this->params['suitid'];
        $productid = $this->params['productid'];
        ...
        for ($i = 1; $i <= 24; $i++)
        {
            ...
            $priceArr = self::get_suit_price($year, $month, $suitid, $typeid, $startdate);
            $out .= empty($priceArr) ? '' : self::calender2($year, $month, $priceArr, $containdiv);
            $month++;
        }

        $this->assign('calendar', $out);
        $this->assign('typeid', $typeid);
        $this->display('pub/calendar2');
    }
    /*
    * 日历筛选
    * */
    public function action_calendar2()
    {
        $typeid = $this->params['typeid'];
        $suitid = $this->params['suitid'];
        $productid = $this->params['productid'];
        ...
        foreach ($arr as $row)
        {
            if ($row)
            {
                ...
                if($typeid=="106"){
                    ...
                    $price[$day]['price'] = isset($row['childprice']) ? $row['childprice'] : $row['price'];
                    $price[$day]['price'] = Currency_Tool::price($price[$day]['price']);
...
';
    ...
    public function calender2($year = '', $month = '', $priceArr = NULL, $contain)
    {
        ...
        //日历头
        $html = '<div id="cald">
               <div class="calhd">' . $year . '年' . $month . '月</div>
              <div class="calwk">
                  <div>一</div>
                  <div>二</div>
                  <div>三</div>
...
';
        ...
        while ($day <= $lastday)
        {
            ...
            {
                $line = '<div class="cal">';
                $line .= str_repeat('<div>&nbsp;</div>', $nowWeek - 1);
            ...
            //单价
            $dayPrice = $priceArr[$cdayme]['price'];
           // $dayPrice = $priceArr[$cdayme]['child_price'];
            /* //成本
             $daybasicprice = $priceArr[$cdayme]['basicprice'];
             //利润
             $dayprofitprice = $priceArr[$cdayme]['profit'];
            ...
            //库存
            $priceArr[$cdayme]['number'] = $priceArr[$cdayme]['number'] < -1 ? 0 : $priceArr[$cdayme]['number'];
            $number = $priceArr[$cdayme]['number'] != -1 ? '余'.$priceArr[$cdayme]['number'] : '';
            ...
            if ($defaultmktime >= $currentmktime)
            {
                if ($dayPrice)
                {
                    ...
                    $tdcontent .= '<i class="red">' .$dayPrice.'元</i>';
                    if ($number === 0)
                    {
                        $onclick = '';
                    }
                    ...
                    $numberinfo = '<i class="green">'.$number.'</i>';
                }
                else
                {
                    $dayPriceStrs = '';
                ...
                if ($onclick == '')
                {
                    ...
                    $line .= "<div><span>" . $tdcontent . "</span></div>";
                }
                else
                {
                    $line .= "<div class='act' $onclick> <span>" . $tdcontent . $numberinfo . "</span></div>";
                }
            }
            else
            {
            ...
            if ($day == $lastday)
            {
                if ($nowWeek != 7)
                {
                    $line .= str_repeat('<div>&nbsp;</div>', 7 - $nowWeek);
                }
                $line .= '</div>';
                $html .= $line;
    ...
    public function action_ajax_calendar3()
    {
        $typeid = $_POST['typeid'];
        $suitid = $_POST['suitid'];
        $productid = $_POST['productid'];
        $containdiv = Arr::get($_POST, 'containdiv');
        $startdate = Arr::get($_POST, 'startdate');//开始日期
        $url = "pub/calendar3/typeid/$typeid/suitid/$suitid/productid/$productid/containdiv/$containdiv/startdate/$startdate";
        $content = Request::factory($url)->execute()->body();
        echo $content;
    }
    public function action_ajax_calendar2()
    {
        $typeid = $_POST['typeid'];
        $suitid = $_POST['suitid'];
        $productid = $_POST['productid'];
        $containdiv = Arr::get($_POST, 'containdiv');
        $startdate = Arr::get($_POST, 'startdate');//开始日期
        $url = "pub/calendar2/typeid/$typeid/suitid/$suitid/productid/$productid/containdiv/$containdiv/startdate/$startdate";
        $content = Request::factory($url)->execute()->body();
        echo $content;
    }
