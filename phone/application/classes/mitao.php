    ...
    public  function  action_member(){
        ...
        $arr = Common::session('openidArr');
            ...
            if(isset($userinfo['errcode'])){
                ...
                $arr = Model_Mitao::GetOpenid2('wxc1ee3ce4c8424f1b','a616e600fa4eaddacb4fa16a8d64af4f');
                Common::session("openidArr",$arr);
                Common::session("openid",$arr['openid']);
                $openid         = $arr['openid'];
                $access_token  = $arr['access_token'];// 有效期 2小时
            ...
            $query = DB::select('id')->from('member_order')->where('openid', '=', $openid)->and_where('status','=','1')->and_where('usedate','>=',$nowtime);
            $noPayNum  = $query->execute()->count();
            $this->assign('noPayNum',$noPayNum);
            $query = DB::select('id')->from('member_order')->where('openid', '=', $openid)->and_where('status','=','2')->and_where('usedate','>=',$nowtime);
            $waitUseNum  = $query->execute()->count();
            $this->assign('waitUseNum',$waitUseNum);
            $query = DB::select('id')->from('member_order')->where('openid', '=', $openid)->and_where('status','=','4');
            $backPayNum  = $query->execute()->count();
            $this->assign('backPayNum',$backPayNum);
            $query = DB::select('id')->from('member_order')->where('openid', '=', $openid)->and_where('status','=','5')->and_where('ispinlun','=','0');
            $noCommNum  = $query->execute()->count();
            $this->assign('noCommNum',$noCommNum);
  ...
  int(1)}
    ...
    *
    ...
    public function action_order_show()
    {
        ...
        $row['total'] = $row['paytype'] == 2 ? ($row['dingnum'] + $row['childnum'] + $row['oldnum']) * $row['dingjin'] : $row['price'] * $row['dingnum'] + $row['childprice'] * $row['childnum'] + $row['oldprice'] * $row['oldnum'];
        //  产品的 详情 url
        $row['purl']  =get_product_info($row['typeid'],$row['productaid']);
        //评论
        ...
        $extendinfo = Model_Mitao::get_extend_info($productDetail['id'],$row['typeid']);
