    ...
    add_order2($arr)
    {
        ...
        if (is_array($arr))
        {
            //会员查询及自动增加
                ...
                $member = Model_Mitao::getUserInfo($arr['openid']);

                if (empty($member))
                {
                    $member['mobile'] = $arr['linktel'];
                    $member['truename'] = $arr['linkman'];
                    $member['openid'] = $arr['openid'];
                    $member_result = Model_Member::register2($member);
                    if (!is_array($member_result))
                    {
                    ...
                    Plugin_Core_Factory::factory()->add_listener('on_member_register',$member)->execute();
                    Common::session("member",$member);
                }else{

                    if(empty($member['mobile'])){

                        $mem['mobile']  = $arr['linktel'];
                        $re= DB::update('member')->set($mem)->where("`openid`='".$arr['openid']."'")->execute();
    ...
    public static function minus_storage($dingdate, $typeid, $suitid, $productid, $dingnum, $departdate = '')
    {
        ...
        switch ($typeid)
        {
            case '1':
                ...
                $sql = "UPDATE `sline_line_suit` SET number=number-$dingnum WHERE lineid='$productid'  AND id='$suitid' AND number!=0 AND number!=-1";
            ...
            case '13':
                ...
                break;
            case '106':
                $sql = "UPDATE `sline_mitao_suit` SET number=number-$dingnum WHERE lineid='$productid' AND id='$suitid' AND number!=0 AND number!=-1";
                DB::query(Database::UPDATE, $sql)->execute();
                break;
        }
