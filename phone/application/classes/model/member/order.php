...
//        if (!is_null($param) && isset($param['isquery']))
//        {
//            $data = self::query_order2($page, $param );
//        }
//        else
//        {
            $data = self::my_order2($mid, $page, $param);
    //    }
        return $data;
    }
        ...
        if (!is_null($param) && $param['type'] != -1)
        {
            switch ($param['type'])
            {
                case 0:
                    ...
                    break;
                case 1:
                    $sql .= "and ((`status`=2) or (`status`=5  and `paytype`=4)) and `isconsume`=0 ";
                    break;
                case 2:
                    $sql .= "and `status` =5 and `ispinlun`=0 and  `isconsume`=1 ";
                    break;
                case 3:
