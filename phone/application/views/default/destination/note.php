...
<header id="_j_search_nav" class="Dest-search-head">
  ...
  <div class="searchNav travel-searchNav">
    <a href="/phone/{$destinfo['pinyin']}/search-{$param}?day={$info['day']}&way={$info['way']}"><i></i>
    <ul class="clearfix">
      <li style="width: 33%;"><span>{$property['mdd']}</span></li>
      <li style="width: 33%;"><span>{$property['day']}</span></li>
      <li style="width: 33%;"><span>{$property['way']}</span></li>
    </ul>
    </a>
  </div>
</header>
...
<div class="no-content" id="no-content" style="text-align: center; display:none; ">
  <img src="{$GLOBALS['cfg_public_url']}images/nocon.png" width="50%"/>
  <p>啊哦，暂时没有相关信息</p>
</div>
<!----没有数据  显示   end  --------->
...
<script type="text/html" id="tpl_article_list">
  ...
  <li onclick="location.href='{{value.url}}'">
    <div class="mask"></div>
    <p class="Note-img"><img src="{{value.litpic}}" height="250"></p>
