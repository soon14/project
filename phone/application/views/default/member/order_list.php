...
<script type="text/html" id="tpl_list_item">
    ...
    <li>
        ...
        <div class="price">
            ...
            <span class="fr"><i data="type"></i><em><i class="currency_sy"></i>{{if value.paytype!='线下支付'}}在线支付{{else}}景区支付{{/if}}</em></span>
        </div>
        ...
        <div class="cool">
            ...
            {{if value.status == 5}}
               {{if value.paytype=='线下支付'}}
                  {{if value.isconsume==1}}
                  <span class="cz"><em>交易成功</em></span>
                  {{else}}
                     {{if value.usedate<value.now_time}}
                      <span class="cz"><em>已过期</em></span>
                     {{else}}
                      <span class="cz"><em>预定成功</em></span>
                     {{/if}}
                  {{/if}}
               {{else}}
            ...
            {{else if value.status == 1}}
                {{if value.paytype=='线下支付'}}
                   {{if value.usedate<value.now_time}}
                    <span class="cz"><em>已失效</em></span>
                   {{else}}
                   <span class="cz"><a class="fk_btn" href="{{value.payurl}}">确认下单</a></span>
                   {{/if}}
                {{else}}
                    {{if value.usedate<value.now_time}}
                    <span class="cz"><em>已失效</em></span>
