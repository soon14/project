	...
	$.fn.lubo=function(options){
		...
		return this.each(function(){
			...
			function cir(){
				...
				cir_box.children(Li).eq(0).css('z-index','1.1');//08.04  fengjishu jia
			...
			function Btn(){
				...
				leftBtn.bind(Click,function(){
					...
					_box.children(Li).eq(onLen-1).stop(false,false).animate({
						...
						_box.children(Li).css('z-index',"0");
						jQuery(this).css("z-index","9");// fengjishu  08.04 加入function 用于控制当前对象的 层级关系
			...
			jQuery(_cirBox).children(Li).bind(Over,function(){
				...
				_box.children(Li).eq(inde).stop(false,false).animate({
					...
					_box.children(Li).css('z-index',"0");
					_box.children(Li).eq(inde).css('z-index','9');// fengjishu  08.04 加入function 用于控制当前对象的 层级关系
				});
