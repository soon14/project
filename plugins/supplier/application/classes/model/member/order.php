...
//==============end
    ...
    public function get_my_order_list($sta=null,$starttime=null,$endtime=null,$paysouce=null)

    {
      ...
      //===============fengjshu 07.22
        $time  =time()*1-86400;
         if($sta!==null&&$sta!==''){
             if($sta==7){//  fengjishu  08.05
                 $whereStr.=  " and ((isconsume=0 and status=5 and paytype=4 and unix_timestamp(usedate)<$time) ";
                 $whereStr.= " or (isconsume=0 and status=2 and paytype!=4 and ispay =1 and unix_timestamp(usedate)<$time))";
                // $whereStr.=" and isconsume=0 and (status ='5' or status='2') and unix_timestamp(usedate)<".$time;
             }elseif($sta==8){
                 $whereStr .= " and status=1 and unix_timestamp(usedate)<$time ";
             }
             else{
                 $whereStr.=" and status ='{$sta}' ";
             }
...

    {
...

        {
            ...
            // 07.14 fengjishu  修改 订单里的名称
            $data['productname'] = $data['productname']?$data['productname']:$product['title'];
...

            {
                ...
                $data['litpic'] = $GLOBALS['cfg_res_url'].'/images/not-img.jpg';
            ...
            $data["usedates"]  = strtotime($data['usedate']);
            //订单状态
            //fengjishu d08.04  start
            if($data['isconsume']==0&&$data['paytype']==4&&$data['status']==5&&$data['usedates']<$time){
                $data['order_status']  = '已过期';
            }elseif($data['isconsume']==0&&$data['paytype']!=4&&$data['status']==2&&$data['usedates']<$time){
                $data['order_status']  = '已过期';
            }elseif($data['status']==1&&$data['usedates']<$time){
                $data['order_status']  = '已失效';
            }
            else{
