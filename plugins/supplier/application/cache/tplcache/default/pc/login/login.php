        ...
        <div class="login-box">
            <div class="login-con">
                ...
                <form id="st_form">
                    ...
                    <div class="user-num" style="display: <?php if($is_code) { ?>block<?php } else { ?>none<?php } ?>
" >
                        <input type="text" class="user-zh" name="code" id="code" placeholder="请输入验证码"/><span class="yzm"><img id="yzm-img" src="<?php echo $cmsurl;?>captcha" onClick="this.src=this.src+'?math='+ Math.random()" ></span><span class="change-next" onClick="document.getElementById('yzm-img').src=document.getElementById('yzm-img').src+'?math='+ Math.random()" >换一张</span>
                    </div>
                    <div class="find-password"><a href="<?php echo $cmsurl;?>pc/findpass">找回密码</a></div>
                    <div class="login-btn"><a class="login-btn" href="javascript:;">登 录</a></div>
                </form>
                <div class="mf-txt">还不是注册商家，<a href="<?php echo $cmsurl;?>pc/register">免费注册</a></div>
            </div>
        </div>
    </div>
</div>
