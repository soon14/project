...
<head>
    <meta charset="utf-8">
    <title>我要验单-<?php echo $webname;?></title>
                        ...
                        <div class="verify-con">
                            <table class="verify-table" width="100%" border="0">
                                ...
                                <tr>
                                    ...
                                    <td><span class="num"><?php echo $v['usedate'];?></span></td>
                                    <td><span class="total">&yen;<?php echo $v['total'];?></span></td>
                                    <td>
                                        <span class="<?php if($v['isconsume']==1 ) { ?>period<?php } else { ?>wxf<?php } ?>
"><?php if($time>$usedate){ ?>已过期<?php }else{?><?php if($v['isconsume']==1 ) { ?>已消费<?php } else { ?>未消费<?php } ?>
<?php }?></span>
                                    </td>
                                    <td>
                                        <a class="show" href="<?php echo $cmsurl;?>pc/checkorder/show_order?id=<?php echo $v['id'];?>">查看订单</a>
                                        <?php if($time>$usedate){ ?><?php }else{?>
                                        <?php if($v['isconsume']!=1 ) { ?><a class="now-yd check-order" href="javascript:;" data-id="<?php echo $v['id'];?>"<?php if($time>$usedate){ ?>style="background: #E3E8F3" <?php }?> >确认验单</a><?php } ?>
                                         <?php }?>
                                    </td>
                                </tr>
                                <?php $n++;}unset($n); } ?>
