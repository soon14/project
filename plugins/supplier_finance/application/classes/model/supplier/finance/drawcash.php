...
class Model_Supplier_finance_Drawcash extends ORM
{
    ...
    public function get_list()
    {
        ...
        $return = $this->_set_pages('supplier_finance_drawcash', $pagesize, $url_array, 'addtime', "supplierid={$st_supplier_id}");
    ...
    public function get_detail($id)
    {
        ...
        $sql = "select * from sline_supplier_finance_drawcash where id={$drawcash_id}";
        $list = $this->_format_data(DB::query(Database::SELECT, $sql)->execute()->as_array());
        return $list;
    }
    public function get_withdrawForSupplier($id)
    {

        $where  = " status=5 and isconsume=1 and paytype!=4 and is_drawcash=0 and find_in_set('$id',supplierlist) ";

        $sql = "select sum(spotprice) as sums from `sline_member_order` where $where";
        $sum =DB::query(Database::SELECT, $sql)->execute()->as_array();
        return $sum[0]['sums'];
       // return $sql;
    }
