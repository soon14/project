...
class Weixin
{
    ...
    private function screen($openid,$picname,$erweiPic){
      //  $url   = 'http://www.aitto.net/phone/spot/activity/huhuwater?openid='.$openid.'&erweiPic='.$erweiPic;
//        $url   = 'http://www.aitto.net/phone/spot/activity/huhuwater';
//       // $save_path ='/home/wwwroot/www.aitto.net/new_uploads/'.$picname.'.png';
//        $save_path ='/home/wwwroot/www.aitto.net/new_uploads/reply/'.time().'.png';
//        $response = exec('/home/phantomjs-2.1.1-linux-x86_64/bin/phantomjs /home/phantomjs-2.1.1-linux-x86_64/examples/rasterize.js '. $url.' '.$save_path);
//        return $save_path;

        $response = exec('/home/phantomjs-2.1.1-linux-x86_64/bin/phantomjs /home/phantomjs-2.1.1-linux-x86_64/examples/rasterize.js http://www.aitto.net/phone/spots/show_64.html /home/wwwroot/www.aitto.net/new_uploads/333.png');
        return '/home/wwwroot/www.aitto.net/new_uploads/reply/'.time().'.png';
    }
    /*
    *  获取 分享活动内容
    */
                    ...
                    if($isimage==1){//  判断 、是否开启了 图片回复功能
                        ...
                        if(!empty($arr)){
                            ...
                            if($time>$starttime&&$time<$overtime){
                                $sql ="select * from `sline_cutPicture_user` where openid ='$toUser'";
                    ...
                    if($s==0||$s==false){//  匹配失败
                        if (!empty($keyword)) {
                            ...
                            if (in_array($keyword, $img_key)) {
                                ...
                                if($keyword=='中奖名单'){
                                    $contentStr = "查看活动中奖名单，点击→http://t.cn/Rfc5Mat";
                                    $msgType = "text";
                                    $resultStr = sprintf($textTpl, $toUser, $fromUser, time(), $msgType, $contentStr);
                                    echo $resultStr;
                                }elseif($keyword=='测试'){
                        ...
                        if(!empty($arr)) {
                           ...
                           // if($keyword=='我要冰瀑门票+手机号')
                            ...
                            if($message['status']==1113){
                                ...
                                // 二维码 图片地址
                                $erweiPic = $this->get_showqrcode("QR_SCENE",$scene_id,604800);
                                // 截屏 图片的保存地址
                                $screen   =$this ->screen($toUser,'2016'.$scene_id,$erweiPic);
    ...
    public function getUsersMes($openid){
        ...
        $this ->insertUserMsgToBBs($openid,$arr);
    }
    //============================
    /*
     * 回复 文本消息
