   ...
   <div class="list-web-ct">
       <table class="list-head-tb">
        ...
        <div class="search-bar filter" id="search_bar">
            ...
            <div class="change-btn-list mt-4 ml-10">
                <span class="change-btn-xz btnbox" id="attrlist" data-url="box/index/type/attrlist/typeid/102" data-result="result_attrlist" >属性&nbsp;&gt;&nbsp;<span id="result_attrlist">全部</span></span>

            </div>

            <div class="pro-search ml-10" style=" float:left; margin-top:4px">
                <input type="text" id="searchkey" value="特产名称" datadef="特产名称" class="sty-txt1 set-text-xh wid_200">
...
<script>
  ...
  Ext.onReady(
    function() 
    {
        ...
        $("#addbtn").click(function(){
            ...
            ST.Util.addTab('添加特产','{$cmsurl}specialty/add/parentkey/article/itemid/10',0);
        });
        ...
        window.product_store=Ext.create('Ext.data.Store',{
         ...
         proxy:{
		   type:'ajax',
		   api: {
              read: SITEURL+'specialty/specialty/action/read',  //读取数据的URL
			  update:SITEURL+'specialty/specialty/action/save',
			  destroy:SITEURL+'specialty/specialty/action/delete'
              },
		      reader:{
                type: 'json',   //获取数据的格式 
                root: 'lists',
							...
							bar.insert(1,Ext.create('Ext.panel.Panel',{border:0,items:[{
                                ...
                                cls:'my-extjs-btn',
								 ...
								 menu:
                                     ...
                                     {text:'属性',handler:function(){ CHOOSE.setSome("设置属性",{loadCallback:setAttrids},SITEURL+'attrid/dialog_setattrid?typeid=102');}},
                                     {text:'专题',handler:function(){ CHOOSE.setSome("设置专题",{loadCallback:setThemes},SITEURL+'theme/dialog_settheme?typeid=102');}},
                                     {text:'图标',handler:function(){  CHOOSE.setSome("设置图标",{loadCallback:setIcons},SITEURL+'icon/dialog_seticon?typeid=102');}}
									]
                            }
  ...
  function updateField(ele,id,field,value,type,callback)
  {
	  ...
	  Ext.Ajax.request({
						 url   :  SITEURL+"specialty/specialty/action/update",
						 method  :  "POST",
						 datatype  :  "JSON",
  ...
  //修改
    ...
    {
        var url = SITEURL+'specialty/edit/parentkey/article/itemid/10/id/'+id;

        parent.window.addTab('修改特产',url,1);
    }
  //官方攻略信息配置
    function goOffical(id,name)
    {
        var url = SITEURL+'specialty/infoset/parentkey/article/itemid/10/specialtyid/'+id;
   ...
   {
       CHOOSE.setSome("设置图标",{loadCallback:setIcons},SITEURL+'icon/dialog_seticon?typeid=102&id='+id,true);
   }
   function setOneAttrids(id)
   {
       CHOOSE.setSome("设置属性",{loadCallback:setAttrids},SITEURL+'attrid/dialog_setattrid?typeid=102&id='+id,true);
   }
   function setOneThemes(id)
   {
       CHOOSE.setSome("设置专题",{loadCallback:setThemes},SITEURL+'theme/dialog_settheme?typeid=102&id='+id,true);
   }
