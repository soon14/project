   ...
   <div class="list-web-ct">
       <table class="list-head-tb">
        ...
        <div class="search-bar filter" id="search_bar">
            ...
            <div class="change-btn-list mt-4 ml-10">
                <span class="change-btn-xz btnbox" id="attrlist" data-url="box/index/type/attrlist/typeid/103" data-result="result_attrlist" >属性&nbsp;&gt;&nbsp;<span id="result_attrlist">全部</span></span>

            </div>

            <div class="pro-search ml-10" style=" float:left; margin-top:4px">
                <input type="text" id="searchkey" value="饭店名称/饭店编号" datadef="饭店名称/饭店编号" class="sty-txt1 set-text-xh wid_200">
...
<script>
    ...
    <?php
		 ...
		 echo  'window.rankmenu='.json_encode(Model_Restaurant_Rank::getList()).';';
  ...
  Ext.onReady(
    function() 
    {
        ...
        $("#addbtn").click(function(){
            var url=SITEURL+"restaurant/add/parentkey/member/itemid/1";
            ST.Util.addTab('添加饭店','{$cmsurl}restaurant/add/parentkey/product/itemid/2',0);
        });
        ...
        window.product_store=Ext.create('Ext.data.Store',{
         ...
         proxy:{
		   type:'ajax',
		   api: {
              read: SITEURL+'restaurant/restaurant/action/read',  //读取数据的URL
			  update:SITEURL+'restaurant/restaurant/action/save',
			  destroy:SITEURL+'restaurant/restaurant/action/delete'
              },
		      reader:{
                type: 'json',   //获取数据的格式 
                root: 'lists',
							...
							bar.insert(1,Ext.create('Ext.panel.Panel',{border:0,items:[{
								 ...
								 menu: {
                                     ...
                                     },
                                     {
                                         ...
                                         CHOOSE.setSome("设置属性", {loadCallback: setAttrids}, SITEURL + 'attrid/dialog_setattrid?typeid=103');
                                     }
                                     },
                                     {
                                         text: '专题', handler: function () {
                                         CHOOSE.setSome("设置专题", {loadCallback: setThemes}, SITEURL + 'theme/dialog_settheme?typeid=103');
                                     }
                                     },
                                     {
                                         text: '图标', handler: function () {
                                         CHOOSE.setSome("设置图标", {loadCallback: setIcons}, SITEURL + 'icon/dialog_seticon?typeid=103');
                                     }
                                     }
                                     ]
                               }
						 ...
						 if(id.indexOf('suit')!=-1)
						   {
                             ...
                             return '<a href="javascript:;"  class="row-add-suit-btn" onclick="ST.Util.addTab(\'添加套餐\',\'restaurant/room_op/parentkey/product/itemid/2/action/add/hotelid/'+id+'\')">添加套餐</a>';
  ...
  {
      var url = SITEURL+'restaurant/edit/parentkey/product/itemid/2/id/'+id;

      parent.window.addTab('修改饭店',url,0);
  }
  //套餐管理
  function roomList(id,hotelname)
  {
      var url = SITEURL+'restaurant/room/parentkey/product/itemid/2/hotelid/'+id;
...
//modSuit
  function modSuit(roomid,hotelid)
  {
      ...
      var url = SITEURL+'restaurant/room_op/parentkey/product/itemid/2/action/edit/hotelid/'+hotelid+'/roomid/'+roomid;
    ...
    {
        CHOOSE.setSome("设置目的地",{loadCallback:setDests,maxHeight:500},SITEURL+'destination/dialog_setdest?typeid=103&id='+id,true);

    }
    function setOneIcons(id)
    {
        CHOOSE.setSome("设置图标",{loadCallback:setIcons},SITEURL+'icon/dialog_seticon?typeid=103&id='+id,true);
    }
    function setOneAttrids(id)
    {
        CHOOSE.setSome("设置属性",{loadCallback:setAttrids},SITEURL+'attrid/dialog_setattrid?typeid=103&id='+id,true);
    }
    function setOneThemes(id)
    {
        CHOOSE.setSome("设置专题",{loadCallback:setThemes},SITEURL+'theme/dialog_settheme?typeid=103&id='+id,true);
    }
</script>
