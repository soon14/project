     ...
     <td valign="top" class="content-rt-td">
         ...
         <div class="yhlb" id="user_lb">
             ...
             <div class="verify-search-con">
                 ...
                 <input type="text" name="keywords" id ="keywords" class="search-txt" style="width: 230px;height: 30px;" placeholder="请输入微信昵称/openid" value="{$keywords}" onkeyup="entersearch()"/>
         ...
         <table class="table" >
          ...
          <tr>
              ...
              <th width="10%">手机号</th>
              <th width="10%">关注时间</th>
              <th width="10%">更新用户信息</th>
              <th width="10%">自驾次数</th>
              <th width="10%">截图活动次数</th>
              <th width="10%">吸粉活动次数</th>
          </tr>
          </thead>
          <tbody>
          {loop $userMes $mes}
          <tr class="tbody " name="tr1">
              ...
              <td with="10%"><img src="{$mes['headimgurl']}" width="60" height="60"></td>
              ...
              <td width="10%" align="center"><a href="JavaScript:" data-openid="{$mes['openid']}"  onclick="updateUserinfo(this)">更新用户信息</a></td>
              <td width="10%" align="center"><a href ='javascript:' data-mid="{$mes['mid']}" onclick="tabb(this,1)">{$mes['drivingRate']}</a></td>
              <td width="10%" align="center"><a href ='javascript:' data-name = "{$mes['nickname']}" data-openid="{$mes['openid']}"  onclick="tabb(this,2)">{$mes['cutpic']}</a></td>
              <td width="10%" align="center"><a href ='javascript:' data-openid="{$mes['openid']}" onclick="tabb(this,3)">{$mes['Powder']}</a></td>
...
<!--              <td width="10%" align="center"><a href ="javascript:" onclick="joinpart(this)" data-openid="{$mes['openid']}">设置</a></td>-->
 ...
 <script>
     ...
     function updateUserinfo(th){
          var openid =  $(th).attr('data-openid');
         $.ajax({
             type:"POST",
             url :"/newtravel/weixin/updateuser",
             data:{openid:openid},
             success: function (data) {
                 alert(data);
             }
     ...
     function tabb(th,num){

         if(num==1){
             var mid = $(th).attr('data-mid');
             var url ='/newtravel/order/index/parentkey/order/itemid/1/typeid/1?Special_key='+mid;
             parent.window.addTab('线路订单',url,0);
         }
         if(num==2){
             var  nickname = $(th).attr('data-name');
             var  openid   =$(th).attr('data-openid');
            var url ='/newtravel/cutpicture/partUser/?openid='+openid;

             parent.window.addTab('截图数据',url,0);
         }
         if(num==3){

             var  openid   =$(th).attr('data-openid');
             var url ='/newtravel/activity/index/?openid='+openid;
             parent.window.addTab('吸粉数据',url,0);
         }
