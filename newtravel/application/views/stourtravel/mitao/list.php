...
<head>
<meta charset="utf-8">
<title>米淘列表-河北旅游网{$coreVersion}</title>
...
<div class="list-top-set">
 ...
 <div class="search-bar filter" id="search_bar">
     ...
     <div class="change-btn-list mt-4  ml-10">
       <span class="change-btn-xz btnbox" id="attrlist" data-url="box/index/type/attrlist/typeid/106" data-result="result_attrlist" >属性&nbsp;&gt;&nbsp;<span id="result_attrlist">全部</span></span>

     </div>
   
   	 <div class="pro-search ml-10 mt-4 fl mt-4">

        <input type="text" id="searchkey" value="活动名称/产品编号/供应商" datadef="线路名称/产品编号/供应商" class="sty-txt1 set-text-xh wid_200">
  ...
  Ext.onReady(
    function() 
    {
        ...
        $("#addbtn").click(function(){
            ...
            ST.Util.addTab('添加活动','{$cmsurl}mitao/add/parentkey/product/itemid/11',0);
        });
        ...
        window.product_store=Ext.create('Ext.data.Store',{
         ...
         proxy:{
		   type:'ajax',
		   api: {
              read: SITEURL+'mitao/mitao/action/read',  //读取数据的URL
			  update:SITEURL+'mitao/mitao/action/save',
			  destroy:SITEURL+'mitao/mitao/action/delete'
              },
		      reader:{
                type: 'json',   //获取数据的格式 
                root: 'lines',
                                ...
                                menu: {
                                    ...
                                    items:[
                                        ...
                                        },
                                        {
                                            ...
                                            CHOOSE.setSome("设置属性", {loadCallback: setAttrids}, SITEURL + 'attrid/dialog_setattrid?typeid=106');
                                         }
                                        },
                                        {
                                            text: '专题',handler: function () {
                                            CHOOSE.setSome("设置专题", {loadCallback: setThemes}, SITEURL + 'theme/dialog_settheme?typeid=106');
                                            }
                                        },
                                        {
                                            text: '图标',handler: function () {
                                            CHOOSE.setSome("设置图标", {loadCallback: setIcons}, SITEURL + 'icon/dialog_seticon?typeid=106');
                                          }
                                        }
                                    ]
                              }
	   ...
	   columns:[
			   ...
			   },
           {
               ...
               renderer: function (value, metadata, record) {
                  ...
                  return "<input type='text' value='"+value+"' class='row-edit-txt' onblur=\"updateField(this,'"+id+"','selfDriMdd',0,'input')\"/>";
               }
           }
//			   ,
//           {
...
//               renderer: function (value, metadata, record) {
//                   //  return  "<input type='checkbox' class='product_check' value='"+value+"'/>";
//                       var  carnum = record.get('carNum');
//                       return '<span >' +carnum + '</span>';
//
//               },
//               listeners:{
//                   afterrender:function(obj,eopts)
...
//               sortable: true,
//               renderer: function (value, metadata, record) {
//                   var  carnum = record.get('carmens') || 0;
//                   return '<span >' +carnum + '</span>';
//               },
//               listeners:{
//                   afterrender:function(obj,eopts)
...
//                   var id=record.get('id');
//
//                   return "<input type='text' value='"+value+"' class='row-edit-txt' onblur=\"updateField(this,'"+id+"','linedate',0,'input')\"/>";
//               }
//           },
               {
                   text: '报价有效期',
					  ...
					  //  return  "<input type='checkbox' class='product_check' value='"+value+"'/>"; 
                                     ...
                                     var name=record.get('title');
			                          if(id.indexOf('suit')!=-1)
                                      {
									     ...
									     return "<a href='javascript:;' class='row-mod-btn' title='修改套餐'  onclick=\"ST.Util.addTab(\'修改套餐\',\'mitao/editsuit/parentkey/product/itemid/11/suitid/"+suitid+"\')\"></a>&nbsp;&nbsp;<a href='javascript:void(0);' title='删除' class='row-del-btn' onclick=\"delSuit('"+id+"')\"></a>";
                                      }
                                      else
                                      {
                                          return '<a href="javascript:;" class="row-add-suit-btn" onclick="ST.Util.addTab(\'添加套餐\',\'mitao/addsuit/parentkey/product/itemid/11/lineid/'+id+'\')">添加套餐</a>';
                                      }
					},				 
				   listeners:{
					    afterrender:function(obj,eopts)
...

			   {
			   ...
			   },
			    {
				   ...
				   renderer : function(value, metadata,record) {
                         ...
                         var linedate=record.get('linedate');// 出行日期
					     var id=record.get('id');//  line线路id

						 return "<a href='javascript:void(0);' title='编辑' class='row-mod-btn' onclick=\"goModify("+id+",'"+linename+"')\"></a>"
                                +"&nbsp;&nbsp;<a href='javascript:void(0);' title='克隆' class='row-clone-btn' onclick=\"ST.Util.goClone("+id+",'mitao')\"></a>"
//                                +"&nbsp;&nbsp;<a href='javascript:void(0);' title='创建订单' class='row-mod-btn' onclick=\"create_order("+id+",'"+linedate+"')\"></a>";
                                   // return getExpandableImage(value, metadata,record);
                    },
					 listeners:{
					    afterrender:function(obj,eopts)
  ...
  {
      parent.window.addTab(linename,SITEURL+'/mitao/edit/lineid/'+lineid+'/parentkey/product/itemid/11',1);
  }
  ...
  {
      CHOOSE.setSome("设置图标",{loadCallback:setIcons},SITEURL+'icon/dialog_seticon?typeid=106&id='+id,true);
  }
  function setOneAttrids(id)
  {
      CHOOSE.setSome("设置属性",{loadCallback:setAttrids},SITEURL+'attrid/dialog_setattrid?typeid=106&id='+id,true);
  }
  function setOneThemes(id)
  {
      CHOOSE.setSome("设置专题",{loadCallback:setThemes},SITEURL+'theme/dialog_settheme?typeid=106&id='+id,true);
  }
  function create_order(id){

        var url = SITEURL +"/mitao/create_order/parentkey/product/itemid/1?lineid="+id;
        ST.Util.addTab('创建自驾订单',url,1);
  }
   function checkorder(th){
       var value  = $(th).attr('data-title');
      // var value1 = value.replace(/[^\u4e00-\u9fa5]/gi,"");
       var url =SITEURL +"/order/index/parentkey/order/itemid/1/typeid/1?keyword="+value;
       ST.Util.addTab('线路订单',url,1);
   }
   function humanCount(aid)
   {
       CHOOSE.setSome("人员统计",{maxHeight:500},SITEURL+'mitao/humanCount?lineaid='+aid,true);
   }
