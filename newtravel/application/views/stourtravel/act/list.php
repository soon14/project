            ...
            <div class="list-web-ct">
                <table class="list-head-tb">
                    <tr>
                        <td class="head-td-lt">
                            {loop $kindmenu $menu}
                            <a href="javascript:;" class="menu-shortcut" onclick="ST.Util.addTab('{$menu['name']}','{$menu['url']}',107);">{$menu['name']}</a>
                            {/loop}
                        </td>
                        <td class="head-td-rt">
...
<!--     </div>-->
     ...
     <div class="change-btn-list mt-4  ml-10">
       <span class="change-btn-xz btnbox" id="attrlist" data-url="box/index/type/attrlist/typeid/107" data-result="result_attrlist" >属性&nbsp;&gt;&nbsp;<span id="result_attrlist">全部</span></span>

     </div>
   
   	 <div class="pro-search ml-10 mt-4 fl mt-4">

        <input type="text" id="searchkey" value="活动名称/编号" datadef="线路名称/产品编号/供应商" class="sty-txt1 set-text-xh wid_200">
...
<!--       <span class="list-4 fl"><a href="javascript:void(0);" title="分销商" onClick="CHOOSE.togMod(this,4)">分销商</a></span>-->
     </span>
   </div> 
  ...
  Ext.onReady(
    function() 
    {
        ...
        $("#addbtn").click(function(){
            ...
            ST.Util.addTab('添加活动','{$cmsurl}act/add/parentkey/product/itemid/12',0);
        });
        ...
        window.product_store=Ext.create('Ext.data.Store',{
         ...
         proxy:{
		   type:'ajax',
		   api: {
              read: SITEURL+'act/act/action/read',  //读取数据的URL
			  update:SITEURL+'act/act/action/save',
			  destroy:SITEURL+'act/act/action/delete'
              },
		      reader:{
                type: 'json',   //获取数据的格式 
                root: 'lines',
...
//                                        },
                                        {
                                            ...
                                            CHOOSE.setSome("设置属性", {loadCallback: setAttrids}, SITEURL + 'attrid/dialog_setattrid?typeid=107');
                                         }
                                        },
//                                        {
...
//                                        }
	   ...
	   columns:[
           ...
           },
           {
               ...
               sortable: true,
               renderer: function (value, metadata, record) {
                   var  carnum = record.get('shownum') || 0;
                   return '<span >' +carnum + '</span>';
               },
               listeners:{
                   afterrender:function(obj,eopts)
...
//			   },
			   ...
			   },
			    {
				   ...
				   renderer : function(value, metadata,record) {
						 ...
						 return "<a href='javascript:void(0);' title='编辑' class='row-mod-btn' onclick=\"goModify("+id+",'"+linename+"')\"></a>"
                                +"&nbsp;&nbsp;<a href='javascript:void(0);' title='克隆' class='row-clone-btn' onclick=\"ST.Util.goClone("+id+",'act')\"></a>";
                                   // return getExpandableImage(value, metadata,record);
                    },
					 listeners:{
					    afterrender:function(obj,eopts)
  ...
  {
      parent.window.addTab(linename,SITEURL+'/act/edit/lineid/'+lineid+'/parentkey/product/itemid/1',1);
  }
  ...
  {
      CHOOSE.setSome("设置目的地",{loadCallback:setDests,maxHeight:500},SITEURL+'destination/dialog_setdest?typeid=107&id='+id,true);

  }
  function setOneIcons(id)
  {
      CHOOSE.setSome("设置图标",{loadCallback:setIcons},SITEURL+'icon/dialog_seticon?typeid=107&id='+id,true);
  }
  function setOneAttrids(id)
  {
      CHOOSE.setSome("设置属性",{loadCallback:setAttrids},SITEURL+'attrid/dialog_setattrid?typeid=107&id='+id,true);
  }
  function setOneThemes(id)
  {
      CHOOSE.setSome("设置专题",{loadCallback:setThemes},SITEURL+'theme/dialog_settheme?typeid=107&id='+id,true);
  }
  function create_order(id){

        var url = SITEURL +"/act/create_order/parentkey/product/itemid/1?actid="+id;
        ST.Util.addTab('创建自驾订单',url,1);
  }
   function checkorder(th){
       var value  = $(th).attr('data-title');
      // var value1 = value.replace(/[^\u4e00-\u9fa5]/gi,"");
       var url =SITEURL +"/order/index/parentkey/order/itemid/1/typeid/107?keyword="+value;
       ST.Util.addTab('线路订单',url,1);
   }
   function humanCount(aid)
   {
       CHOOSE.setSome("人员统计",{maxHeight:500},SITEURL+'act/humanCount?lineaid='+aid,true);
   }
