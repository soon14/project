...
class Controller_Restaurant extends Stourweb_Controller
{
    ...
    public function before()
    {
        ...
        {
            Common::getUserRight('restaurant', 'sadd');// 判断当前是否有操作权限
        }
        else if ($action == 'edit')
        {
            Common::getUserRight('restaurant', 'smodify');
        }
        else if ($action == 'price')
        {
            Common::getUserRight('restaurantprice', 'slook');
        }
        else if ($action == 'ajax_price')
        {
            ...
            if ($op == 'save')
            {
                Common::getUserRight('restaurantprice', 'smodify');
            }
            else if ($op == 'del')
            {
                Common::getUserRight('restaurantprice', 'sdel');
            }
        }
        else if ($action == 'rank' || $action == 'ajax_ranklist')
        {
            Common::getUserRight('restaurantrank', 'slook');
        }
        else if ($action == 'ajax_rank_add')
        {
            Common::getUserRight('restaurantrank', 'sadd');
        }
        else if ($action == 'ajax_rank_save')
        {
            Common::getUserRight('restaurantrank', 'smodify');
        }
        else if ($action == 'ajax_rank_del')
        {
            Common::getUserRight('restaurantrank', 'sdel');
        }
        $this->assign('parentkey', $this->params['parentkey']);
        $this->assign('itemid', $this->params['itemid']);
        $this->assign('weblist', Common::getWebList());
        $this->assign('templetlist', Common::getUserTemplteList('restaurant_show'));//获取上传的用户模板
    }
    ...
    */
    public function action_restaurant()
    {
        ...
        {
            $this->assign('kindmenu', Common::getConfig('menu_sub.restaurantkind'));//分类设置项
            $this->display('stourtravel/restaurant/list');
        }
...
,ifnull(b.displayorder,9999) as displayorder,ifnull(d.suitday,0) as suitday from sline_restaurant as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=2) left join (select c.hotelid,c.id,min(c.suitday) as suitday from(select a.hotelid,a.id,max(b.day) as suitday
 from sline_restaurant_room a left join sline_restaurant_room_price b on a.id=b.suitid  group by a.id) c group by c.hotelid) d on a.id=d.hotelid  where $w $order limit $start,$limit";
            }
            else
            {
...
,a.themelist,b.isding,b.isjian,b.istejia,ifnull(b.displayorder,9999) as displayorder,ifnull(d.suitday,0) as suitday from sline_restaurant as a left join sline_kindorderlist as b on (b.classid=$kindid and a.id=b.aid and b.typeid=2) left join (select c.hotelid,c.id,min(c.suitday) as suitday from(select a.hotelid,a.id,max(b.day) as suitday
 from sline_restaurant_room a left join sline_restaurant_room_price b on a.id=b.suitid  group by a.id) c group by c.hotelid) d on a.id=d.hotelid where $w $order limit $start,$limit";
            }
            $totalcount_arr = DB::query(Database::SELECT, "select count(*) as num from sline_restaurant a where $w")->execute()->as_array();
            ...
            foreach ($list as $k => $v)
            {
                ...
                $homeSql = "select a.*,ifnull(max(b.day),0) as suitday from sline_restaurant_room a left join sline_restaurant_room_price b on a.id=b.suitid where a.hotelid={$v['id']}  group by a.id $suitOrder";
        ...
        else if ($action == 'delete') //删除某个记录
        {
            ...
            if (is_numeric($id)) //酒店
            {
                $model = ORM::factory('restaurant', $id);
                $model->deleteClear();
            }
            else if (strpos($id, 'suit') !== FALSE)
            {
                $suitid = substr($id, strpos($id, '_') + 1);
                $suit = ORM::factory('restaurant_room', $suitid);
                $restaurantid = $suit->hotelid;
                $suit->deleteClear();
                Model_Restaurant::updateMinPrice($restaurantid);
            }
        }
        else if ($action == 'update')//更新某个字段
        {
            ...
            else  //如果不是排序字段
            {
                ...
                {
                    $suitid = substr($id, strpos($id, '_') + 1);
                    $model = ORM::factory('restaurant_room', $suitid);
                }
                if ($model->id)
                {
                    $model->$field = $val;
                    ...
                    {
                        $model->$field = implode(',', Model_Attrlist::getParentsStr($val, 103));
                    }
                    $model->save();
                    if ($model->saved())
                        echo 'ok';
    ...
    {
        $columns = ORM::factory('restaurant_content')->where("(webid=0 and isopen=1) or columnname='tupian'")->order_by('displayorder', 'asc')->get_all();
        $this->assign('columns', $columns);
        $this->assign('webid', 0);
        $this->assign('position', '添加饭店');
        $this->assign('action', 'add');
        $this->assign('ranklist', ORM::factory('restaurant_rank')->getList());
        $this->display('stourtravel/restaurant/edit');
    }
    ...
    public function action_edit()
    {
      ...
      // $ranklist    =  ORM::factory('restaurant_rank')->getList();
        $this->assign('ranklist', ORM::factory('restaurant_rank')->getList());

        $info = ORM::factory('restaurant', $productid)->as_array();
        $columns = ORM::factory('restaurant_content')->where("(webid=0 and isopen=1) or columnname='tupian'")->order_by('displayorder', 'asc')->get_all();
        $info['kindlist_arr'] = Model_Destinations::getKindlistArr($info['kindlist']);
        $info['attrlist_arr'] = Common::getSelectedAttr(103, $info['attrid']);
        ...
        $extendinfo = Common::getExtendInfo(103, $info['id']);
        $this->assign('columns', $columns);
        $this->assign('extendinfo', $extendinfo);//扩展信息
        $this->assign('info', $info);
        $this->assign('position', '修改饭店' . $info['title']);
        $this->display('stourtravel/restaurant/edit');
    }
    ...
    public function action_ajax_save()
    {
        ...
        {
            $model = ORM::factory('restaurant');
            $model->aid = Common::getLastAid('sline_restaurant', $webid);
            $model->addtime = time();
        }
        else
        {
            ...
            {
                $aid = Common::getLastAid('sline_restaurant', $webid);
                $model->aid = $aid;
            }
            $productid = $id;
        }
        ...
        if (!empty($attrids))
        {
            $attrmode = ORM::factory("restaurant_attr")->where("id in ($attrids)")->group_by('pid')->get_all();
        ...
        $model->content = $link->keywordReplaceBody(Arr::get($_POST, 'jieshao'), 103);
        ...
        if ($model->saved())
        {
            ...
            Common::saveExtendData(103, $productid, $_POST);//扩展信息保存
            $status = true;
        }
    ...
    {
        $this->display('stourtravel/restaurant/rank');
    }
    ...
    {
        $model = new Model_Restaurant_Rank();
    ...
    public function action_ajax_rank_add()
    {
        ...
        $model = ORM::factory('Restaurant_rank');
    ...
    public function action_ajax_rank_save()
    {
        ...
        {
            $obj = ORM::factory('restaurant_rank')->where('id', '=', $id[$i])->find();
    ...
    public function action_ajax_rank_del()
    {
        ...
        $model = ORM::factory('restaurant_rank', $navid);
        $model->delete();
        $out = array();
        if (!$model->loaded())
        {
    ...
    public function action_price()
    {
        ...
        if (empty($action))
        {
            $list = ORM::factory('restaurant_pricelist')->where('webid', '=', '0')->order_by('min', 'asc')->get_all();
            $this->assign('list', $list);
            $this->display('stourtravel/restaurant/price');
        }
        if ($action == 'save')
        {
    ...
    * */
    public function action_ajax_price()
    {
        ...
        if ($action == 'save')
        {
            ...
            {
                $obj = ORM::factory('restaurant_pricelist')->where('id', '=', $id[$i])->find();
                $obj->min = $min[$i];
                $obj->max = $max[$i];
                $obj->update();
                $obj->clear();
            ...
            {
                $obj = ORM::factory('restaurant_pricelist');
                $obj->min = $newmin[$i];
                $obj->max = $newmax[$i];
                $obj->webid = 0;
                $obj->create();
        ...
        {
            $id = Arr::get($_POST, 'id');
            $model = ORM::factory('restaurant_pricelist', $id);
            $model->delete();
            $out = array();
            if (!$model->loaded())
            {
    ...
    public function action_room()
    {
        ...
        {
            $info = ORM::factory('restaurant', $hotelid)->as_array();
            $position = $info['title'];
            $this->assign('position', $position);
            $this->display('stourtravel/restaurant/room');
        }
        else if ($action == 'read')    //读取列表
        {
            $start = Arr::get($_GET, 'start');
            ...
            /*$list = ORM::factory('hotel_room')
                       ...
                       ->limit($limit)
                       ->as_array();*/
            $sql = "select a.* from sline_restaurant_room a where a.hotelid='$hotelid' order by a.id desc limit $start,$limit";
            $totalcount_arr = DB::query(Database::SELECT, "select count(*) as num from sline_restaurant_room a where hotelid='$hotelid'")->execute()->as_array();
            $list = DB::query(Database::SELECT, $sql)->execute()->as_array();
            $new_list = array();
            foreach ($list as $k => $v)
            {
        ...
        else if ($action == 'delete') //删除某个记录
        {
            ...
            if (is_numeric($id)) //
            {
                $model = ORM::factory('restaurant_room', $id);
                $model->delete();
            }
        }
    ...
    public function action_room_op()
    {
        ...
        $hotelid = $this->params['hotelid'];
        $this->assign('hotelid', $hotelid);
        $hotelinfo = ORM::factory('restaurant', $hotelid)->as_array();
        $this->assign('restaurantname', $hotelinfo['title']);
        if ($action == 'add')
        {
        ...
        else if ($action == 'edit')
        {
            ...
            $info = ORM::factory('restaurant_room', $roomid)->as_array();
        ...
        $this->display('stourtravel/restaurant/room_edit');
    }
    ...
    public function action_ajax_room_save()
    {
        ...
        {
            $model = ORM::factory('restaurant_room');
        }
        else //修改保存
        {
            $model = ORM::factory('restaurant_room', $roomid);
        }
    ...
    public function saveBaoJia($hotelid, $roomid, $arr)
    {
        ...
        if ($pricerule == 'all')
        {
            ...
            while (true)
            {
                $model = ORM::factory('restaurant_room_price')->where("suitid=$roomid and day='$begintime'")->find();
                $data_arr = array();
                ...
                if ($model->suitid)
                {
                    $query = DB::update('restaurant_room_price')->set($data_arr)->where("suitid=$roomid and day='$begintime'");
                    $query->execute();
                }
                else
                {
            ...
            while (true)
            {
                ...
                foreach ($monthval as $v)
                {
                    ...
                    if ((int)$v > (int)$daynum || $newtime < $stime || $newtime > $etime)
                        continue;
                    $model = ORM::factory('restaurant_room_price')->where("suitid=$roomid and day='$newtime'")->find();
                    $data_arr = array();
                    $data_arr['hotelid'] = $hotelid;
                    $data_arr['suitid'] = $roomid;
                    $data_arr['basicprice'] = $basicprice;
                    ...
                    if ($model->suitid)
                    {
                        $query = DB::update('restaurant_room_price')->set($data_arr)->where("suitid=$roomid and day='$newtime'");
                        $query->execute();
                    }
                    else
                    {
        ...
        Model_Restaurant::updateMinPrice($hotelid);
    }
