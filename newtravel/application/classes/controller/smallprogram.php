...
class Controller_Smallprogram  extends Stourweb_Controller{
	...
	public function action_article()
	{
		...
		if(empty($action))  //显示线路列表页
		{
		   ...
		   $this->display('stourtravel/smallprogram/list');
		}
		else if($action=='read')    //读取列表
		{
			...
			if(empty($kindid))
			{
			  $sql="select a.*,ifnull(b.displayorder,999999) as displayorder from sline_smallprogram as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=105)where $w $order limit $start,$limit";
			}
			else
			{
			   $sql="select a.* from sline_smallprogram as a where $w $order limit $start,$limit";

			}
			$totalcount_arr=DB::query(Database::SELECT,"select count(*) as num  from sline_smallprogram  a  where $w")->execute()->as_array();
...
//                $v['end_time'] = date("Y-m-d H:i:s",$v['end_time']);
		...
		$rawdata=file_get_contents('php://input');
		   ...
		   if(is_numeric($id))
		   {
		    $model=ORM::factory('smallprogram',$id);
		    $model->deleteClear();
		   }
			...
			if($field=='displayorder')  //如果是排序
			{
			    ...
			    if(is_numeric($id))//
				{
				    if(empty($kindid))  //全局排序
				   {
					 ...
					 $order_mod=$order->where("aid='$id' and typeid=105 and webid=0")->find();
    ...
    public function action_add()
    {
        ...
        $this->display('stourtravel/smallprogram/edit');
    }
    /*
    * 修改页面
    * */
    public function action_edit()
    {
        ...
        $info = ORM::factory('smallprogram',$productid)->as_array();

        $info['piclist_arr'] =  json_encode(Common::getUploadPicture($info['piclist']));//图片数组

        $this->assign('info',$info);
        $this->assign('position','修改内容'.$info['title']);
        $this->display('stourtravel/smallprogram/edit');
    ...
    public function action_ajax_save()
    {
        ...
        $content = Arr::get($_POST,'bzcontent');//文章内容
        ...
        {
            $model = ORM::factory('smallprogram');
        ...
        $model->start_time =Arr::get($_POST,'start_time');

        $model->end_time = Arr::get($_POST,'end_time');


        $model->litpic = $litpic;
        
        $model->share_title = Arr::get($_POST,'share_title');

        $model->sponsor =Arr::get($_POST,'sponsor');
        ...
        echo json_encode(array('status'=>$status,'productid'=>$productid,"s"=>$model->ishidden));
    ...
    public function action_ajax_saves()
    {
        ...
        $sql = "select count(*) as alls from `sline_article_mulu` where pid=0 and articleid ='$id' ";
        ...
        if ($is_show) {
            ...
            $imgsss  = trim($imgsss,",");
            $piclist   = ORM::factory('smallprogram',$id)->get('piclist');
            if(!empty($piclist)){
    ...
    * */
    public function action_ajax_del_attach()
    {
        ...
        $doc = ORM::factory('smallprogram',$id)->get('attachment');
