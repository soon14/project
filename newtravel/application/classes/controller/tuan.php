...
class Controller_Tuan extends Stourweb_Controller{
	...
	public function action_tuan()
	{
		...
		else if($action=='read')    //读取列表
		{
			...
			if(empty($kindid))
			{
			  $sql="select a.id,a.webid,a.aid,a.title,a.endtime,a.shownum,a.finaldestid,a.bookcount,a.totalnum,a.virtualnum,a.validdate,a.attrid,a.kindlist,a.jifenbook,a.jifentprice,a.jifencomment,a.iconlist,a.supplierlist,a.themelist,b.isjian,b.isding,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.iconlist,''))=0,0,1) as order_iconlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist,
...
b.isjian,b.isding,ifnull(b.displayorder,9999) as displayorder,ishidden from sline_tuan as a left join sline_kindorderlist as b on (b.classid=$kindid and a.id=b.aid and b.typeid=13)  where $w $order limit $start,$limit";
   ...
   */
    ...
    public function action_ajax_tuansave()
    {
        ...
        if(empty($totalnum))
        {
            $totalnum=$totalnum===0||$totalnum==='0'?0:-1;
        }
        else
        {
