...
class Controller_Act extends Stourweb_Controller
...

    {
...

        {
            ...
            if (!empty($user_action))
                ...
                Common::getUserRight('act', $user_action);
...

        {
            ...
            Common::getUserRight('act', 'smodify');
...

        {
            ...
            if (!empty($user_action))
                ...
                Common::getUserRight('actday', $user_action);
...

        {
            ...
            if (!empty($user_action))
                ...
                Common::getUserRight('actprice', $user_action);
...

        {
            ...
            if (!empty($user_action))
                ...
                Common::getUserRight('actjieshao', $user_action);
    ...
    */
...

    {
...

        {
            ...
            $this->assign('kindmenu', Common::getConfig('menu_sub.actkind'));//分类设置项

            $this->display('stourtravel/act/list');
...
//  加入 linedate  值  1205 加入 selfDriMdd
                $sql = "select a.id,a.aid,a.title,a.iconlist,a.linedate,a.timeover,a.selfDriMdd,a.finaldestid,a.shownum,a.price,a.startcity,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.iconlist,''))=0,0,1) as order_iconlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist,

a.attrid,a.webid,a.kindlist,a.ishidden,a.piclist,a.themelist,a.supplierlist,b.isjian,IFNULL(b.displayorder,9999) as displayorder,b.isding,b.istejia,ifnull(d.suitday,0) as suitday from sline_act as a left join sline_kindorderlist b on (a.id=b.aid and b.typeid=107 and b.classid=$kindid)  left join (select

c.lineid,c.id,min(c.suitday) as suitday from(select a.lineid,a.id,max(ifnull(b.day,0)) as suitday from sline_act_suit a left join sline_act_suit_price b on a.id=b.suitid group by a.id) c group by c.lineid) d on a.id=d.lineid where $w $order limit  $start,$limit";
            ...
            {
                 //  加入 linedate  值  1205 加入 selfDriMdd
                $sql = "select a.id,a.aid,a.title,a.supplierlist,a.linedate,a.timeover,a.selfDriMdd,a.shownum,a.iconlist,a.finaldestid,a.price,a.startcity,a.attrid,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.iconlist,''))=0,0,1) as order_iconlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist,a.webid,a.kindlist,a.ishidden,a.piclist,a.themelist,b.isjian,IFNULL(b.displayorder,9999) as displayorder,b.isding,b.istejia,ifnull(d.suitday,0) as suitday from sline_act as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=107)   left join (select

c.lineid,c.id,min(c.suitday) as suitday from(select a.lineid,a.id,max(ifnull(b.day,0)) as suitday from sline_act_suit a left join sline_act_suit_price b on a.id=b.suitid group by a.id) c group by c.lineid) d on a.id=d.lineid where $w $order  limit $start,$limit";

            }

            $totalcount_arr = DB::query(Database::SELECT, "select count(*) as num from sline_act a where $w")->execute()->as_array();
...

            {
                ...
                $v['attrname'] = Model_act_Attr::getAttrnameList($v['attrid']);

                $v['url'] = Common::getBaseUrl($v['webid']) . '/acts/show_' . $v['aid'] . '.html';
                ...
                $suitSql = "select a.*,max(b.day) as suitday from sline_act_suit a left join sline_act_suit_price b on a.id=b.suitid where a.lineid={$v['id']}  group by a.id $suitOrder";
...

                {
                    ...
                    $val['minprice'] = Model_act_Suit_Price::getMinPrice($val['id']);

                    $val['minprofit'] = Model_act_Suit_Price::getMinPrice($val['id'], 'adultprofit');
...

        {
...

            {
...

                {
...

                    {
                        ...
                        $order_mod = $order->where("aid='$id' and typeid=107 and webid=0")->find();
...

                    {
                        ...
                        $kindorder_mod = $kindorder->where("aid='$id' and typeid=107 and classid=$kindid")->find();
...

            {
                ...
                $suit = ORM::factory('act_suit', $suitid);
...

        {
...

            {
                ...
                $line = ORM::factory('act', $id);

                $line->deleteClear();

                $jieshao_model = new Model_Act_Jieshao();
...

            {
                ...
                Model_act::updateMinPrice($lineid);
...

        {
...

            {
...

                {
                    ...
                    $model = ORM::factory('act_suit', $suitid);
...

    {
        ...
        $columns = ORM::factory('act_content')->where("(webid=" . $webid . " and isopen=1 and isline=0 and columnname!='linespot') or columnname='tupian' ")->order_by('displayorder', 'asc')->get_all();
        ...
        $this->display('stourtravel/act/edit');
...

    {
       ...
       // $startplacelist = ORM::factory('startplace')->where("pid!=0")->get_all();
...

        {
            ...
            $day_arr = array_chunk(ORM::factory('act_jieshao')->where("lineid='" . $info['id'] . "'")->order_by('day', 'asc')->get_all(), $info['lineday']);

            $info['linejieshao_arr'] = $day_arr[0];

            $columns = ORM::factory('act_content')->where("(webid=0 and isopen=1 and isline=0 and columnname!='linespot') or (columnname='tupian' and webid=0)")->order_by('displayorder', 'asc')->get_all();
    ...
    */
...

    {
...

        {
            ...
            $attrids = implode(',', Model_Attrlist::getParentsStr($attrids, 107));
        ...
        $data_arr['timeover'] = Arr::get($_POST, 'timeover');
        //=========fengjishu  end
        ...
        $data_arr['multi_period'] = Arr::get($_POST, 'multi_period') ? Arr::get($_POST, 'multi_period') : 0;//是否为多期
        ...
        $images_arr1      = Arr::get($_POST, 'images1');

        $imagestitle_arr1 = Arr::get($_POST, 'images1title');
...

        {
            ...
            $model = ORM::factory('act');

            $model->aid = Common::getLastAid('sline_act', $data_arr['webid']);
...

        {
...

            {
                ...
                $aid = Common::getLastAid('sline_act', $data_arr['webid']);
...

        {
           ...
           // $this->savejieshao($lineid);
            ...
            Common::saveExtendData(107, $lineid, $_POST);//扩展信息保存
    ...
    public  function  action_delimg_luoji(){
          ...
          $lines   = ORM::factory("act",$lineid)->as_array();
    ...
    public  function  action_changePrice(){
        ...
        $distributorid      =   Arr::get($_POST, 'distributorid');

        $typeid      =   Arr::get($_POST, 'typeid');

        $proid     =   Arr::get($_POST, 'proid');

        $price     =   Arr::get($_POST, 'price');

        $re = Common::updateDistribution($typeid,$distributorid,$proid,$price);
...

    {
...

        {
            ...
            $model = ORM::factory('act_jieshao')->where("lineid='$lineid' and day='$k'")->find();

            if (empty($model->id))

                $model = ORM::factory('act_jieshao');
    ...
    public function action_clearMore(){
        ...
        $sql  = "DELETE FROM `sline_act_suit_price` where suitid=$suitid and lineid=$productid";
...

    {
...

        {
            ...
            $list = ORM::factory('act_day')->get_all();

            $this->assign('list', $list);

            $this->display('stourtravel/act/day');
...

    {
...

        {
            ...
            $list = ORM::factory('act_pricelist')->get_all();

            $this->assign('list', $list);

            $this->display('stourtravel/act/price');
...

        {
            ...
            $model = ORM::factory('act_pricelist');
...

        {
...

            {
                ...
                $model = ORM::factory('act_pricelist', $k);
...

        {
            ...
            $model = ORM::factory('act_pricelist', $id);
...

    {
...

        {
            ...
            $this->display('stourtravel/act/attr');
...

        {
...

            {
                ...
                $list = ORM::factory('act_attr')->where('pid=0')->get_all();
...

            {
                ...
                $list = ORM::factory('act_attr')->where('pid=' . $node)->get_all();
...

        {
            ...
            $movemodel = ORM::factory('act_attr', $moveid);

            $overmodel = ORM::factory('act_attr', $overid);
    ...
    */
...

    {
...

        {
            ...
            $linecontent = ORM::factory('act_content')->where('webid=0')->order_by('displayorder')->get_all();

            $this->assign('list', $linecontent);

            $this->display('stourtravel/act/content');
...

        {
...

            {
                ...
                $model = ORM::factory('act_content', $k);
...

    {
        ...
        $lineinfo = ORM::factory('act', $lineid)->as_array();
        ...
        $this->display('stourtravel/act/suit_edit');
   ...
   */
...

    {
        ...
        $info = ORM::factory('act_suit', $suitid)->as_array();
        ...
        $lineinfo = ORM::factory('act', $info['lineid'])->as_array();
...

    {
...

        {
...

            {
                ...
                $model = ORM::factory('act_suit_price')->where("suitid=$suitid and day='$begintime'")->find();
...

                {
                    ...
                    $query = DB::update('act_suit_price')->set($data_arr)->where("suitid=$suitid and day='$begintime'");
...

        {
...

            {
...

                {
                    ...
                    $model = ORM::factory('act_suit_price')->where("suitid=$suitid and day='$newtime'")->find();
...

                    {
                        ...
                        $query = DB::update('act_suit_price')->set($data_arr)->where("suitid=$suitid and day='$newtime'");
...

    {
...

        {
...

            {
                ...
                $data=DB::select()->from('act')->where('id', '=', $_POST['id'])->execute()->current();
                ...
                if(!empty($data)){
                    ...
                    DB::update('act')->set(array('linedoc' => serialize($attach)))->where('id', '=', $_POST['id'])->execute();
