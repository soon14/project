...
class Controller_Destination extends Stourweb_Controller
{
    ...
    private $product_arr = array(1 => 'line', 2 => 'hotel', 3 => 'car', 4 => 'article', 5 => 'spot', 6 => 'photo', 13 => 'tuan',102=>'specialty');
    private $name_arr = array(1 => '线路', 2 => '酒店', 3 => '租车', 4 => '文章', 5 => '景点', 6 => '相册', 13 => '团购',102=>'特产');
 ...
 *   添加标签
 */
    public function action_addtags(){
        // 目的的id
        $kindid  = trim($_POST['kindid']);

        $sql ="insert into `sline_destinations_tag` (`kindid`) value ('$kindid')";
        DB::query(Database::SELECT, $sql)->execute();

        $sql = "select id from `sline_destinations_tag` where kindid =$kindid ";
        $tagids = DB::query(Database::SELECT, $sql)->execute()->as_array();
        $all  =count($tagids)*1-1;
        $tagid  = $tagids[$all]['id'];
     ...
     *
     */
    public  function action_editdisplay(){
        $tagid  = trim($_POST['tagid']);
        $tagdisplay  = trim($_POST['tagdis']);
        $sql ="  UPDATE `sline_destinations_tag` SET `display`='$tagdisplay' WHERE id=$tagid";
        DB::query(1,$sql)->execute();
    }
    /*
     *   修改 目的的季节设置
     */
    public  function action_change_season(){
        ...
        $val     = trim($_POST['val']);
        $model = ORM::factory('destinations', $kindid);
        $model ->$name = $val;
        $flag = $model ->save();
        if($flag){
   ...
   */
    public  function action_edittag(){
        $tagid  = trim($_POST['tagid']);
        $tagname  = trim($_POST['tagdis']);
        $sql ="  UPDATE `sline_destinations_tag` SET `tag_name`='$tagname ' WHERE id=$tagid";
        DB::query(1,$sql)->execute();
    }
...
*/
    public function action_deltag(){
        // 目的的id
        $id  = trim($_GET['id']);
        $sql  = "delete from `sline_destinations_tag` where id =$id ";
        DB::query(1,$sql)->execute();
        echo $id;
        //  echo json_encode($tag);
    }
    ...
    */
    ...
    public function action_dialog_basicinfo()
    {
        ...
        $info = $model->as_array();
        //feng  获取 标签内容
        $sql  = "select * from `sline_destinations_tag` where kindid=$id";
        $tags = DB::query(1,$sql)->execute()->as_array();
