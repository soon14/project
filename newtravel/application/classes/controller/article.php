    ...
    * */
    public function action_edit()
    {
       ...
       //=========fengjishu  09.01
        ...
        $info['mulu']=DB::query(1,$sql)->execute()->as_array();

         /*
          *   对于  content 的处理 是 因为含有 img  中的双引号 放在 编辑器
          *   的initialContent参数  特殊字符 会冲突
          *   这样  要对 content 进行转译处理
          *   Common::addxiegang
          *
          *
          * */
        $info['mulu']=$this->unlimitedForLayer ($info['mulu'] ,$pid = 0 , $child = 'child', $parentField='pid', $primarykey='id');
        //对 content 的处理
        foreach($info['mulu'] as &$v){
            $v['content'] =Common::addxiegang($v['content']);

            foreach ($v['child'] as &$l){
                $l['content'] = Common::addxiegang($l['content']);
            }
        }
    ...
    public function action_ajax_save()
    {
        ...
        $spotlist = Arr::get($_POST,'spotlist');
        ...
        $model->spotlist = implode(',',$spotlist);//所属目的地
        ...
        //  是否 推荐
        $model->isrecommend = Arr::get($_POST,'isrecommend');
        $model->modtime = time();
    ...
    public function action_ajax_saves()
    {
        ...
        if ($is_show) {
            ...
            $imgsss  = trim($imgsss,",");
            $piclist   = ORM::factory('article',$id)->get('piclist');
            if(!empty($piclist)){
