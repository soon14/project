...
class Controller_Attrid extends Stourweb_Controller{
    ...
    public function action_content()
    {
        ...
        if(empty($action))
        {
            ...
            if($modeuleinfo['issystem']!=1)
            {
                $contentlist=$model->where('webid','=',0)->and_where('typeid','=',$typeid.' and columnname!="tupian"')->order_by('displayorder')->get_all();
            }
            else{
                $contentlist = $model->where('webid=0 and columnname!="tupian"')->order_by('displayorder')->get_all();
             }
