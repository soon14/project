    ...
    public function action_savemsg()
    {
        ...
        if($msgtype!='reg')
        {
            for ($i = 1; $i <= 4; $i++)
            {
                ...
                $currmsgtype = $msgtype . $i;

                $tmparr = DB::query(Database::SELECT, "select * from sline_sms_msg  where msgtype='{$currmsgtype}'")->execute()->as_array();
                if (count($tmparr) <= 0)
                    $sql = "insert into sline_sms_msg(msgtype,msg,isopen) values('{$currmsgtype}','{$msg}','{$open}')";
                else
                    $sql = "update sline_sms_msg set isopen='{$open}',msg='{$msg}' where msgtype='{$currmsgtype}'";
