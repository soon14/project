...

{
   ...
   */
    ...
    public function  action_humanCount(){
      ...
      //产品信息
      $lineid     = ORM::factory('line')->where("aid=$productAid")->find('id');

      $typeid     = 1;

      $re         =  Model_Line::humanCount($productAid,$typeid);
      ...
      $this ->display('stourtravel/line/humanCount');
 ...
 * */
...

    {
       ...
       $carinfo = array(
        ...
        if(Model_Line::add_order($arr, $carinfo, $wxuserid,$tourer))

        {
            $orderInfo = Model_Line::get_order_by_ordersn($orderSn);
...
c.lineid,c.id,min(c.suitday) as suitday from(select a.lineid,a.id,max(ifnull(b.day,0)) as suitday from sline_line_suit a left join sline_line_suit_price b on a.id=b.suitid group by a.id) c group by c.lineid) d on a.id=d.lineid where $w $order limit  $start,$limit";
            ...
            {
                 //  加入 linedate  值  1205 加入 selfDriMdd
                $sql = "select a.id,a.aid,a.title,a.supplierlist,a.linedate,a.selfDriMdd,a.iconlist,a.finaldestid,a.price,a.startcity,a.attrid,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.iconlist,''))=0,0,1) as order_iconlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist,a.webid,a.kindlist,a.ishidden,a.piclist,a.themelist,b.isjian,IFNULL(b.displayorder,9999) as displayorder,b.isding,b.istejia,ifnull(d.suitday,0) as suitday from sline_line as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=1)   left join (select
...
c.lineid,c.id,min(c.suitday) as suitday from(select a.lineid,a.id,max(ifnull(b.day,0)) as suitday from sline_line_suit a left join sline_line_suit_price b on a.id=b.suitid group by a.id) c group by c.lineid) d on a.id=d.lineid where $w $order  limit $start,$limit";
...

            {
                ...
                //增加 车辆总数  和 报名人总数数
                $v['carNum']   = Model_Line::get_carNum($v['id']);
                // 人数
                $v['carmens']      =Model_Line::get_carmens($v['id']);
    ...
    */
    ...
    public  function  action_delimg_luoji(){
          ...
          $lines   = ORM::factory("line",$lineid)->as_array();
    ...
    public function action_clearMore(){
        ...
        $sql  = "DELETE FROM `sline_line_suit_price` where suitid=$suitid and lineid=$productid";
