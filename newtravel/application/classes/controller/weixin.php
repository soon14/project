...
class Controller_Weixin extends Stourweb_Controller
{
    ...
    public function action_index(){
        ...
        $jk_model  =  Arr::get($_GET,'option');

        $jk_model   = empty($jk_model) ? "服务号"  : $jk_model ;
    ...
    public function action_imageNews(){
        ...
        $wx_id    = Common::remove_xss(Arr::get($_GET,'wx_id'));
        $wx_id    = empty($wx_id) ? 24 :$wx_id;
        $sql           ="SELECT * FROM `sline_wximagenews` where wx_id=".$wx_id;
    ...
    public function  action_addimageNews(){
        ...
        $wx_id          =  Common::remove_xss(Arr::get($_POST,'wx_id'));
        $time           =time();
        if($update_id==-1){
        $sql            ="INSERT INTO `sline_wximagenews`(`keywords`, `title`, `content`, `pic`, `url`, `type`, `state`, `sort`, `is_openid`, `add_time`,`wx_id`) VALUES ('$keywords','$title','$description','$imgAddress','$imgLinkAddress','暂时未用','未用','$sort','$status','$time',$wx_id)";
        $s              =  $model -> query($sql);
    ...
    public function action_zdy_menu(){
        ...
        $sql              = "SELECT * FROM `sline_wxaddmenu` where pid=0 and wx_id=".$wx_id;
        $s                = $this ->model->get_sql($sql,1);
        $sql              = "select * from `sline_wxaddmenu` where pid!=0 and wx_id=".$wx_id;
        $r                = $this ->model-> get_sql($sql,1);
        $this ->assign("wx_id",$wx_id );
    ...
    public function action_menuCreate(){
     ...
     $model            = new Model_Nav();
      if($up_id==-1){
          if($p_id!=-2){
              $sql              = "INSERT INTO `sline_wxaddmenu`(`pid`, `type`, `title`, `content`, `status`,`wx_id`) VALUES ('$p_id','$type','$title','$content','$status',$wx_id)";
              $s                = $model->query($sql);
              if($s==1)echo "添加子菜单成功";

          }
          else{
              $sql              = "INSERT INTO `sline_wxaddmenu`(`pid`, `type`, `title`, `content`, `status`,`wx_id`) VALUES ('$pid','$type','$title','$content','$status',$wx_id)";
              $s                = $model->query($sql);
