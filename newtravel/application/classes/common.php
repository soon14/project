...

{
    ...
    //匹配内容里面的图片 2017.01.17 封
    public static  function  get_contentImg($content){

        preg_match_all('/<[img|IMG].*?src=[\'|\"](.*?(?:[\.gif|\.jpg|\.png|\.jpeg]))[\'|\"].*?[\/]?>/', $content, $con_img);
        //$con_img 有连个数组 0,1 ；0是带有 img 标签的 的数组 1 是 仅有 图片地址的数组
        return $con_img;
    }
    ...
    */
...

    {
        ...
        $productattr_arr = array(1 => 'line_attr', 2 => 'hotel_attr', 3 => 'car_attr', 4 => 'article_attr', 5 => 'spot_attr', 6 => 'photo_attr', 13 => 'tuan_attr',102=>'specialty_attr',103=>'restaurant_attr',104=>'smallprogram_question_attr',106=>'mitao_attr');

        $attrtable = $typeid < 14||$typeid==102||$typeid==103||$typeid==104||$typeid==106? $productattr_arr[$typeid] : 'model_attr';
...

        {
            ...
            if ($typeid < 14||$typeid==102||$typeid==103||$typeid==104||$typeid==106)
    ...
    public static function message($msg)
    {
        ...
        $javascript = Common::getScript('jquery.min.js,layer/layer.m.js');
        echo <<<EOT
        <!DOCTYPE html>
        <html lang="en">
        <head>
...
EOT;
    ...
    *   $model 1 为商家  2 为 个人
    */
    public  static  function  getDistributor($typeid,$proid,$model=1){


         $sql ="select a.*,b.dis_price from `sline_distributor` as a LEFT JOIN `sline_distributionprice` as b on a.id=b.distributorid and b.typeid=$typeid and b.proid=$proid where a.verifystatus=3 ORDER by a.displayorder asc";
    ...
    public  static  function updateDistribution($typeid,$distributorid,$proid,$price=0){

         if(empty($typeid)||empty($distributorid)||empty($proid)){

             return array();
             exit;

         }
         $sql  = "select id from `sline_distributionprice` where typeid=$typeid and distributorid=$distributorid and proid=$proid";
         $row = DB::query(1, $sql)->execute()->as_array();
         if(empty($row)){
                 $m= ORM::factory("distributionprice");
             $m->typeid=$typeid;
             $m->distributorid=$distributorid;
             $m->proid=$proid;
         }else{
                 $m= ORM::factory("distributionprice",$row[0]['id']);
         }
         $m->dis_price=$price;
         $flag = $m->save();
         if($flag){
...

    {
        ...
        //    return true;
        //===========fengjishu 2016.12.10
        if($arr['supplierlist']=='9'){
            //self::add_eticketno($ordersn);
            require(DOCROOT . '/public/vendor/meijing/base.php');
            $testObj  = new Base();
             ...
             */
            $time          = date("Y-m-d H:i:s",$arr['addtime']);
            $linktel       = trim($arr['linktel']);
            $num           = trim($arr['dingnum']);
            $goods         = trim($arr['partner_number']);
            $linkman       = trim($arr['linkman']);
            $remark        = trim($arr['remark']);
...
//
          ...
          //  $rs = $testObj->submitOrder($goodsId,$num,$order_note,$forecasttime,$outOrderId,$phone,$guest_name,$identityno,$consignee,$address,$zipcode,$isR=0,$meberInfo=array());
            ...
            fwrite($file,date("Y-md H:i:s",time())."  ".$ordersn."的验证码是:".$testObj->s.$rs['body']['orderId'].',第三方订单号：'.$rs['body']['orderId'].',代理商订单号:'.$rs['body']['outOrderId'].',合作伙伴编号:'.$arr['partner_number'].",数量".$arr['dingnum'].',返回数量：'.$rs['body']['inCount'].",联系人电话".$arr['linktel'].",联系人".$arr['linkman'].',备注'.$arr['remark']."\r\n");
            fclose($file);
            //self::add_eticketno($ordersn);
            /*
              1：问题: 验证吗 是否验证相同 ？
...
/*
 *  取消美景订单  丰技术 2016.1210
 *
 * $ordersn 订单号
 */
    public  static  function  cancleMJOrder($ordersn,$outBackId){
        //=========07.29 fengjishu
        include_once(BASEPATH.'/api/meijing/base.php');
        $testObj  = new Base();

        // d
        $third_orderid = ORM::factory('member_order')->where("ordersn='$ordersn'")->find()->get('third_ordersn');
...

    {
        ...
        /*
         * 路劲  上 本地 为'/public/vendor/msg.class.php'
         *         网上 为'public/vendor/msg.class.php'
         *
         */
...
//    }
    ...
    public static  function  read_log($data,$file,$type='a+'){
        ...
        $file = !empty($file) ? APPPATH .$file : APPPATH .'text_log.txt';
    ...
    * */
...

            {
...

                {
...

                    {
                        ...
                        if (in_array($k, array('line', 'hotel', 'car', 'spot', 'visa', 'restaurant','tuan','mitao','act')))
    ...
    public static function add_eticketno($ordersn,$sup)
...
//        }
        ...
        DB::update('member_order')->where('ordersn', '=', "{$ordersn}")->set(array('eticketno' => $sup))->execute();
