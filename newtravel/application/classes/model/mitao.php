...
class Model_Mitao extends ORM {
    ...
    protected  $_table_name = 'mitao';
    
	public function deleteClear()
	{
		 //DB::delete('line_suit_price')->where("suitid={$this->id}")->execute();
		 $suits=ORM::factory('mitao_suit')->where("lineid={$this->id}")->find_all()->as_array();
   ...
   */
    ...
    public static function suit_price($suitId,$useDate)
        ...
        $arr = ORM::factory('mitao_suit_price')
...
//                }
    ...
    private function insertDaySpot($lineid,$spotname,$litpic,$day,$spotid)
    {
        ...
        $sql="select count(*) as num from sline_mitao_dayspot where lineid='$lineid' and title='$spotname' and day=$day";

        $row = $this->query($sql,1);
        $flag = 0;
        if($row[0]['num']==0)
        {
            $sql="insert into sline_mitao_dayspot(lineid,title,spotid,litpic,day) values('$lineid','$spotname','$spotid','$litpic','$day')";
    ...
    {
        $sql = "delete from sline_mitao_dayspot where id= '$autoid'";
    ...
    {
        $sql = "select * from sline_mitao_dayspot where lineid='$lineid' and day='$day'";
    ...
    public function cloneLine($id, $num)
    {
        ...
        for($i = 1; $i <= $num; $i++)
        {
            $newaid=Common::getLastAid('sline_mitao',0);//新线路aid
            $arr['aid']=$newaid;
    ...
    {
        $sql = "select * from sline_mitao_jieshao where lineid='$oldlineid'";
        ...
        foreach($arr as $row)
        {
            ...
            $sql="INSERT INTO sline_mitao_jieshao (";
            $sql2="VALUES ( ";
            $sql_key = '';
            $sql_value = '';
   ...
   * */
...

    {
...

        {
            ...
            case '1':
                ...
                $sql = "UPDATE `sline_mitao_suit_price` SET number=number-$dingnum WHERE day='$day' AND suitid='$suitid' AND number!=0 AND number!=-1";
    ...
    * */
    ...
    {
	    $newtime = time();
        $sql = "SELECT MIN(adultprice) as price FROM sline_mitao_suit_price WHERE lineid='$lineid' and adultprice>0 and day>=$newtime";
