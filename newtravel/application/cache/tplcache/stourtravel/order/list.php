...
<head>
    <meta charset="utf-8">
    <title>问答管理-思途CMS<?php echo $coreVersion;?></title>
    ...
    <tr>
        ...
        <td valign="top" class="content-rt-td" style="overflow:hidden">
            ...
            <div class="search-bar filter" id="search_bar">
                ...
                <div class="change-btn-list mt-4 ml-10">
                    <select name="supplier" onchange="togSupplier(this)" class="bar-select">
                        <option value="">供应商</option>
                        <?php $n=1; if(is_array($supplier)) { foreach($supplier as $v) { ?>
                        <option name='suppliername' value="<?php echo $v['id'];?>" <?php if($v['id']==$supid) { ?>selected=selected<?php } ?>
><?php echo $v['suppliername'];?></option>
                        <?php $n++;}unset($n); } ?>
                    </select>
                </div>
                <div class="change-btn-list mt-4 ml-10">
                    <select name="distributor" onchange="togDistributor(this)" class="bar-select">
                        <option value="">分销商</option>
                        <?php $n=1; if(is_array($distributor)) { foreach($distributor as $v) { ?>
                        <option name='distributorname' value="<?php echo $v['id'];?>" <?php if($v['id']==$dispid) { ?>selected=selected<?php } ?>
><?php echo $v['distributorname'];?></option>
                        <?php $n++;}unset($n); } ?>
                    </select>
                </div>
                <div class="pro-search ml-10 mt-4 fl mt-4">
                    <?php if($typeid ==1) { ?>
                      <?php if($keyword) { ?>
                      <input type="text" id="searchkey" value="<?php echo $keyword;?>" placeholder="订单号/产品名称/联系人" datadef="订单号/产品名称/联系人" class="sty-txt1 set-text-xh wid_200"  onkeydown="entersearch()"/>
                      <?php } else { ?>
...
<input type="hidden" value="<?php echo $Special_key;?>" id ="skey">
</table>
<script>
        ...
        */
        ...
        window.product_store = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                api: {
                    read: SITEURL+'order/index/action/read/typeid/<?php echo $typeid;?>?supplier_id=<?php echo $supid;?>',  //读取数据的URL
             ...
             // ===============fengjishu    end
                ...
                },
                {
                    ...
                    renderer: function (value, metadata, record) {
                        ...
                        if(typeid=='1'){
                            var wxuserid  = record.get('wxuserId');
                        }
                        if(!isNaN(id))
                        {
                            var html="<select ";
                              if(value==7||value==8) {
                                  ...
                                  html += "onchange=\"updateField(this," + id + ",'status',0,'select',"+wxuserid+")\"";
                              }
                                html+=" class='row-edit-select'><option>所有</option>";
                ...
                },
                {
                    ...
                    renderer: function (value, metadata, record) {
                        ...
                        var html = "<a href='javascript:void(0);' onclick=\"view(" + id + ","+typeid+")\">"+ico+"</a>";
                         if(typeid=='1') {
                             html += "&nbsp;&nbsp;<a href='javascript:void(0);' onclick=\"cheview(" + id + ","+typeid+")\">" + ico + "</a>";
                         }
                        return html;
...
function togSupplier(ele)
{
    var supid=$(ele).val();//供应商的ｉｄ
    window.product_store.getProxy().setExtraParam('supplier_id',supid);
    window.product_store.loadPage(1);
}
function togDistributor(ele)
{
    var supid=$(ele).val();//分销商的ｉｄ
    window.product_store.getProxy().setExtraParam('distributor_id',supid);
    window.product_store.loadPage(1);
}
function togSupplier_1()
{
    var supid=$('option[selected=selected]').val();//供应商的ｉｄ
    window.product_store.getProxy().setExtraParam('supplier_id',supid);
    window.product_store.loadPage(1);
}
$(function(){
...
function searchKeyword(keywords) {
    ...
    window.product_store.getProxy().setExtraParam('keyword',keyword);
    window.product_store.getProxy().setExtraParam('Special_key',0);
    window.product_store.loadPage(1);
}
function searchKeyword2(keywords) {
        var  Special_key =  "<?php echo $Special_key;?>";
        var keyword = $.trim($("#searchkey").val());
        var datadef = $("#searchkey").attr('datadef');
        keyword = keyword==datadef ? '' : keyword;
        keyword  = keyword=='' ? keywords :keyword;
        keyword  =  Special_key =='' ? keyword : Special_key;
        window.product_store.getProxy().setExtraParam('Special_key',keyword);
        window.product_store.loadPage(1);
    }
//切换每页显示数量
...
//更新某个字段
function updateField(ele, id, field, value, type,wxuserid) {
    var record = window.product_store.getById(id.toString());
    //this,"+id+",'status',0,'select'
    if (type == 'select') {
        value = Ext.get(ele).getValue();
    }
    var view_el = window.product_grid.getView().getEl();
    var typeid = "<?php echo $typeid;?>"; // 11.23封  加入 用于 线路取消订单 时候 操作 微信用户表
    Ext.Ajax.request({
        url: SITEURL+"order/index/action/update/",
        method: "POST",
        datatype: "JSON",
        params: {id: id, field: field, val: value, kindid: 0,typeid:typeid,wxuserid:wxuserid},
...
//查看线路订单 车的详情
function cheview(id,typeid)
{
    var record = window.product_store.getById(id.toString());
    var url=SITEURL+"order/cheview/id/"+id+"/typeid/"+typeid;
    //ST.Util.showBox('查看订单信息',url,600,300,function(){window.product_store.load()});
    ST.Util.addTab('订单:'+record.get('ordersn')+'车主信息',url,1);
}
</script>
</body>
</html>
<script type="text/javascript" src="http://update.souxw.com/service/api_V3.ashx?action=releasefeedback&ProductName=stourwebcms&Version=5.0.201605.1201&DomainName=&ServerIP=unknown&SerialNumber=44433670" ></script>
